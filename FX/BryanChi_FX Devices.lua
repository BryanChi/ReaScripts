...
cache = {}
Draw =
	{ Rect = {}, DrawMode = {}, ItemInst = {}, L = {}, R = {}, Y = {}, T = {}, B = {}, FxGUID = {}, Time = 0, Df_EdgeRound = {} }
AddFX = { Pos = {}, Name = {}, GUID = {} }
DelFX = { Pos = {}, Name = {} }
...
----track--------------------------------
Trk =
	{ GUID = {}, Prm = { FXGUID = {}, Inst = {}, AssignWhich = {}, V = {}, O_V = {}, Num = {}, WhichMcros = {} }, FxGUID = {}, PreFX = {} }
...
function explode_rgba(rgba)
	return ((rgba >> 24) & 0xFF) / 255, ((rgba >> 16) & 0xFF) / 255, ((rgba >> 8) & 0xFF) / 255, (rgba & 0xFF) / 255
end
...
function magnitude_to_01(m, freq)
	...
	-- Tilt around 1kHz
	if tilt ~= 0.0 then
		db = db + tilt * ((math.log(freq) / math.log(2)) - (math.log(1024) / math.log(2)))
	end
...
for i = 1, 340, 1 do -- 340 is width
	iToFreq = x_to_freq(i)
	if iToFreq > 50 and iToFreq < 51 then
		iPos50 = i
	end
	if iToFreq > 99 and iToFreq < 102 then
		iPos100 = i
	end
	if iToFreq > 198 and iToFreq < 201 then
		iPos200 = i
	end
	if iToFreq > 490 and iToFreq < 500 then
		iPos500 = i
	end
	if iToFreq > 990 and iToFreq < 1020 then
		iPos1k = i
	end
	if iToFreq > 1980 and iToFreq < 2010 then
		iPos2k = i
	end
	if iToFreq > 4900 and iToFreq < 5050 then
		iPos5k = i
	end
	if iToFreq > 9990 and iToFreq < 10300 then
		iPos10k = i
	end
end
...
BlackListFXs = {
	"Macros",
	"JS: Macros .+",
	"Frequency Spectrum Analyzer Meter",
	"JS: FXD Split to 32 Channels",
	...
	"AU: AULowpass (Apple)",
	"AU: AULowpass",
	"VST: FabFilter Pro C 2 ",
	"Pro-C 2",
	"Pro C 2",
	"JS: FXD Split to 4 channels",
	"JS: FXD Gain Reduction Scope",
	"JS: FXD Saike BandSplitter",
	"JS: FXD Band Joiner",
	"FXD Saike BandSplitter",
	"FXD Band Joiner",
	...
	if Str then
		...
		if End and Str and LineChange then
			...
			elseif Type == "Bool" then
				if string.sub(Str, End + 1, LineChange - 1) == "true" then
					Out = true
				else
					Out = false
				end
...
function GetProjExt_FxNameNum(FxGUID)
	...
	if PrmCount ~= "" then
		FX.Prm.Count[FxGUID] = tonumber(PrmCount)
	end
...
function SyncAnalyzerPinWithFX(FX_Idx, Target_FX_Idx, FX_Name)
	...
	if L ~= Target_L then
		if not FX_Name then
			_, FX_Name = r.TrackFX_GetFXName(LT_Track, FX_Idx)
		end
...
function ToggleCollapseAll()
	...
	for i = 0, Sel_Track_FX_Count - 1, 1 do
		if not FX[FXGUID[i]].Collapse then
			All_Collapsed = false
		end
	end
...
function PrepareFXforModulation(FX_Idx, P_Num, FxGUID)
	...
	if
		r.TrackFX_AddByName(LT_Track, "FXD Macros", 0, 0) == -1
		and r.TrackFX_AddByName(LT_Track, "Macros", 0, 0) == -1
	then
...
function HighlightSelectedItem(
	...
	if Foreground == "Foreground" then
		WinDrawList = Glob.FDL
	else
		WinDrawList = Foreground
	end
	if not WinDrawList then
		WinDrawList = r.ImGui_GetWindowDrawList(ctx)
	end
	if FillClr then
		r.ImGui_DrawList_AddRectFilled(WinDrawList, L, T, R, B, FillClr)
	end

	if OutlineClr and not rounding then
		r.ImGui_DrawList_AddLine(WinDrawList, L - P, T - P, L - P, T + h / VSC - P, OutlineClr)
		r.ImGui_DrawList_AddLine(WinDrawList, R + P, T - P, R + P, T + h / VSC - P, OutlineClr)
		r.ImGui_DrawList_AddLine(WinDrawList, L - P, B + P, L - P, B + P - h / VSC, OutlineClr)
		r.ImGui_DrawList_AddLine(WinDrawList, R + P, B + P, R + P, B - h / VSC + P, OutlineClr)
		r.ImGui_DrawList_AddLine(WinDrawList, L - P, T - P, L - P + w / HSC, T - P, OutlineClr)
		r.ImGui_DrawList_AddLine(WinDrawList, R + P, T - P, R + P - w / HSC, T - P, OutlineClr)
		r.ImGui_DrawList_AddLine(WinDrawList, L - P, B + P, L - P + w / HSC, B + P, OutlineClr)
		r.ImGui_DrawList_AddLine(WinDrawList, R + P, B + P, R + P - w / HSC, B + P, OutlineClr)
	else
		if FillClr then
			r.ImGui_DrawList_AddRectFilled(WinDrawList, L, T, R, B, FillClr, rounding)
		end
		if OutlineClr then
			r.ImGui_DrawList_AddRect(WinDrawList, L, T, R, B, OutlineClr, rounding)
		end
	end
	if GetItemRect == "GetItemRect" then
		return L, T, R, B, w, h
...
function BlinkItem(dur, rpt, var, highlightEdge, EdgeNoBlink, L, T, R, B, h, w)
	...
	if highlightEdge then
		EdgeClr = highlightEdge
	end
...
function MyText(text, font, color, WrapPosX)
	if WrapPosX then
		r.ImGui_PushTextWrapPos(ctx, WrapPosX)
	end
...
function MakeItemEditable(FxGUID, Fx_P, ItemWidth, ItemType, PosX, PosY)
	if FX.LayEdit == FxGUID and Draw.DrawMode[FxGUID] ~= true then
		local DeltaX, DeltaY = r.ImGui_GetMouseDelta(ctx)
		...
		if LE.Sel_Items and not r.ImGui_IsAnyItemActive(ctx) then
			if r.ImGui_IsKeyPressed(ctx, r.ImGui_Key_DownArrow()) and Mods == 0 then
				for i, v in ipairs(LE.Sel_Items) do
					if v == Fx_P then
						FX[FxGUID][v].PosY = FX[FxGUID][v].PosY + LE.GridSize
					end
				end
			...
			elseif r.ImGui_IsKeyPressed(ctx, r.ImGui_Key_LeftArrow()) and Mods == 0 then
				for i, v in ipairs(LE.Sel_Items) do
					if v == Fx_P then
						FX[FxGUID][v].PosX = FX[FxGUID][v].PosX - LE.GridSize
					end
				end
			...
			elseif r.ImGui_IsKeyPressed(ctx, r.ImGui_Key_DownArrow()) and Mods == Shift then
				for i, v in ipairs(LE.Sel_Items) do
					if v == Fx_P then
						FX[FxGUID][v].PosY = FX[FxGUID][v].PosY + 1
					end
				end
			...
			elseif r.ImGui_IsKeyPressed(ctx, r.ImGui_Key_LeftArrow()) and Mods == Shift then
				for i, v in ipairs(LE.Sel_Items) do
					if v == Fx_P then
						FX[FxGUID][v].PosX = FX[FxGUID][v].PosX - 1
					end
				end
		...
		elseif ItemType == "Knob" or (not ItemType and FX.Def_Type[FxGUID] == "Knob") then
			...
			if MouseX > R - 5 and MouseX < R + 5 and MouseY > B - 5 and MouseY < B + 3 then
				...
				if IsLBtnClicked then
					...
					for i, v in pairs(LE.Sel_Items) do
						if v == Fx_P then
							ChangeSelItmRadius = true
						end
					end
					if ChangeSelItmRadius then
						LE.ChangeRadius = "Group"
					else
						LE.ChangeRadius = Fx_P
					end
				end
			end
		end
		...
		function ChangeParamWidth(Fx_P)
			...
			if
				ItemType == "Sldr"
				or ItemType == "V-Slider"
				or ItemType == "Drag"
				or ItemType == "Selection"
				or ItemType == "Switch"
			then
				FX[FxGUID][Fx_P].Sldr_W = ItemWidth
			end
			if LBtnRel and ChangePrmW == Fx_P then
				FX[FxGUID][Fx_P].Sldr_W = roundUp(FX[FxGUID][Fx_P].Sldr_W, LE.GridSize)
			end
			if LBtnRel then
				ChangePrmW = nil
			end
		...
		function ChangeKnobRadius(Fx_P)
			...
			local Dx, Dy = r.ImGui_GetMouseDelta(ctx)
			if not FX[FxGUID][Fx_P].Sldr_W then
				FX[FxGUID][Fx_P].Sldr_W = Df.KnobRadius
			end
			...
			if LBtnRel and LE.ChangeRaius == Fx_P then
				FX[FxGUID][Fx_P].Sldr_W = roundUp(FX[FxGUID][Fx_P].Sldr_W, LE.GridSize / 2)
			end
			if LBtnRel then
				LE.ChangeRadius = nil
			end
		...
		function ChangeItmPos()
			if LBtnDrag then
				...
				FX[FxGUID][Fx_P].PosX = FX[FxGUID][Fx_P].PosX + Dx
				FX[FxGUID][Fx_P].PosY = FX[FxGUID][Fx_P].PosY + Dy
		...
		if LBtnRel and LE.ChangePos == Fx_P and Max_L_MouseDownDuration > 0.1 then
			if
				(Mods ~= Shift and Mods ~= Shift + Ctrl and Mods ~= Shift + Alt)
				and FX[FxGUID][Fx_P].PosX
				and FX[FxGUID][Fx_P].PosY
			then
				FX[FxGUID][Fx_P].PosX =
					SetMinMax(roundUp(FX[FxGUID][Fx_P].PosX, LE.GridSize), 0, Win_W - (FX[FxGUID][Fx_P].Sldr_W or 15))
...
function AddSwitch(LT_Track, FX_Idx, Value, P_Num, BgClr, Lbl_Type, Fx_P, F_Tp, FontSize, FxGUID)
	...
	if FontSize then
		Font = "Font_Andale_Mono_" .. roundUp(FontSize, 1)
		r.ImGui_PushFont(ctx, _G[Font])
	end
	if FX[FxGUID][Fx_P].Lbl_Clr then
		r.ImGui_PushStyleColor(ctx, r.ImGui_Col_Text(), FX[FxGUID][Fx_P].Lbl_Clr)
	end
	local popClr
	...
	if FP.V_Clr then
		r.ImGui_PushStyleColor(ctx, r.ImGui_Col_Text(), FP.V_Clr)
	end
	...
	elseif FP.Lbl_Pos == "Free" then
		...
		r.ImGui_DrawList_AddTextEx(
			...
			_G[Font],
			FontSize or 11,
			Cx + (FP.Lbl_Pos_X or 0),
			Cy + (FP.Lbl_Pos_Y or 0),
			FP.Lbl_Clr or getClr(r.ImGui_Col_Text()),
	...
	if BgClr then
		r.ImGui_PopStyleColor(ctx, popClr)
	end
	if FX[FxGUID][Fx_P].Lbl_Clr then
		r.ImGui_PopStyleColor(ctx)
	end
	...
	if #OPs == 0 then
		for i = 0, 1, 0.01 do
			...
			if Value ~= buf then
				OPs[#OPs + 1] = buf
				V[#V + 1] = i
				local L1 = r.ImGui_CalcTextSize(ctx, buf)
				local L2 = r.ImGui_CalcTextSize(ctx, Value)
...
function AddCombo(
	...
	if Fx_P and FP then
		if (FP.Lbl_Pos == "Left" and Lbl_Pos ~= "No Lbl") or FP.Lbl_Pos == "Top" then
			...
			if not LabelOveride and not FP.CustomLbl and not CustomLbl then
				_, name = r.TrackFX_GetParamName(LT_Track, FX_Idx, WhichPrm)
			end
			r.ImGui_AlignTextToFramePadding(ctx)
			MyText(
				LabelOveride or FP.CustomLbl or CustomLbl or FP.Name,
				_G[Font],
	...
	if LabelOveride then
		_G[LabelValue] = LabelOveride
	end
	...
	if Style == "Pro C 2" then
		...
		PopClr = 2
		if _G[LabelValue] == "Mastering" then
			_G[LabelValue] = "Master"
		end
	else
	...
	function begincombo(ctx)
		...
		if
			r.ImGui_BeginCombo(
				ctx,
				"##" .. tostring(Label),
				LabelOveride or _G[LabelValue],
				r.ImGui_ComboFlags_NoArrowButton()
			)
		then
			-----Style--------
					...
					if r.ImGui_Selectable(ctx, Options[i], i) and WhichPrm ~= nil then
						if OptionValues then
							...
							r.TrackFX_SetParamNormalized(
								...
								FX_Idx,
								WhichPrm,
								(i - 1) / #Options + ((i - 1) / #Options) * 0.1
							) -- + options* 0.05 so the value will be slightly higher than threshold,
						end
						if FX[FxGUID][Fx_P].ManualValues then
	...
	if not FX[FxGUID][Fx_P].Sldr_W then
		FX[FxGUID][Fx_P].Sldr_W, H = r.ImGui_GetItemRectSize(ctx)
	end
...
function Link_Param_to_CC(
	...
	if Baseline_V then
		ParmModTable["PARAMOD_BASELINE"] = Baseline_V
	end
	if Scale then
		ParmModTable["PARMLINK_SCALE"] = Scale
	end
...
function GetParmModTable(TrkNum, FX_Idx, P_Num, TableIndex_Str)
	...
	if tab then
		TblIDReturn = tab[TableIndex_Str or "PARAM_TYPE"]
	end
...
function Unlink_Parm(TrackNumToBeMod, FX_Slt_Indx_ToBeMod, PARAM_Num)
	...
	alteredFXStateChunk =
		ultraschall.DeleteParmModFromFXStateChunk(FXStateChunk, FX_Slt_Indx_ToBeMod + 1, PARAM_Num + 1)
...
local function Filter_actions(filter_text)
	...
	local t = {}
	if filter_text == "" or not filter_text then
		return t
	end
	for i = 1, #FX_LIST do
...
function FilterBox(FX_Idx, LyrID, SpaceIsBeforeRackMixer, FxGUID_Container, SpcIsInPre, SpcInPost, SpcIDinPost)
	...
	if AddLastSPCinRack then
		FX_Idx_For_AddFX = FX_Idx - 1
	end
	...
	local function InsertFX(Name)
		...
		if SpcIsInPre then
			...
			for i, v in pairs(Trk[TrkID].PreFX) do
				r.GetSetMediaTrackInfo_String(LT_Track, "P_EXT: PreFX " .. i, v, true)
			end
	...
	if ADDFX_FILTER ~= "" and ADDFX_FILTER then
		...
		if x + MAX_FX_SIZE > VP_R then
			x = ParentWinPos_x - MAX_FX_SIZE
		end

		r.ImGui_SetNextWindowPos(ctx, x, y - filter_h / 2)
		if
			r.ImGui_BeginPopup(ctx, "##popupp", r.ImGui_WindowFlags_NoFocusOnAppearing() --[[ MAX_FX_SIZE, filter_h ]])
		then
...
function attachImages()
	Img = {
		Analog1 = r.ImGui_CreateImage(
			r.GetResourcePath() .. "/Scripts/ReaTeam Scripts/FX/BryanChi_FX Devices/Images/Analog Knob 1.png"
		),
	}
...
function DrawModLines(Macro, AddIndicator, McroV, FxGUID, F_Tp, Sldr_Width, P_V, Vertical, FP)
	...
	if AddIndicator and FP.ModAMT[Macro] ~= 0 then
		if Trk[TrkID].Mod[Macro].Type == "env" then
			...
			if Vertical ~= "Vert" then
				ModPosWithAmt =
					math.min(SliderCurPos + (EnvVal * Sldr_Width * FP.ModAMT[Macro]) or 0, PosX_End_Of_Slider)
			else
			...
			if McroV and McroV ~= 0 then
				if Vertical ~= "Vert" then
					ModPosWithAmt =
						math.min(SliderCurPos + (McroV * (FP.ModAMT[Macro] * Sldr_Width) or 0), PosX_End_Of_Slider)
				else
					ModPosWithAmt =
						math.max(SliderCurPos - (McroV * (FP.ModAMT[Macro] * Sldr_Width) or 0), PosX_End_Of_Slider)
				end
				if Vertical ~= "Vert" then
					r.ImGui_DrawList_AddRectFilled(
						drawlist,
...
function IfTryingToAddExistingPrm(Fx_P, FxGUID, Shape, L, T, R, B, Rad)
	if Fx_P .. FxGUID == TryingToAddExistingPrm then
		if
			r.time_precise() > TimeNow and r.time_precise() < TimeNow + 0.1
			or r.time_precise() > TimeNow + 0.2 and r.time_precise() < TimeNow + 0.3
		then
...
function IconBtn(w, h, icon, BGClr, center, Identifier) -- Y = wrench
	...
	L, T, R, B, W, H = HighlightSelectedItem(
		FillClr,
		0x00000000,
		0,
		L,
...
function RestoreBlacklistSettings(FxGUID, FX_Idx, LT_Track, PrmCount)
	...
	if FXsBL == "Has Blacklist saved to FX" then -- if there's FX-specific BL settings
		...
		for i = 0, PrmCount - 4, 1 do
			...
			_, FX[FxGUID].PrmList[i].BL =
				r.GetSetMediaTrackInfo_String(LT_Track, "P_EXT: Morph_BL" .. FxGUID .. i, "", false)
			if FX[FxGUID].PrmList[i].BL == "Blacklisted" then
				FX[FxGUID].PrmList[i].BL = true
			else
				FX[FxGUID].PrmList[i].BL = nil
			end
		end
...
function AddKnob(
	...
	if V_Pos == "Free" then
		...
		r.ImGui_DrawList_AddTextEx(
			draw_list,
			Arial,
			FX[FxGUID][Fx_P].V_FontSize or Knob_DefaultFontSize,
			pos[1] + (FP.V_Pos_X or 0),
			pos[2] + (FP.V_Pos_Y or 0),
			FX[FxGUID][Fx_P].V_Clr or 0xffffffff,
			FormatPV,
			(Radius or 20) * 2
		)
	end
	...
	if FP.Lbl_Pos == "Free" then
		...
		r.ImGui_DrawList_AddTextEx(
			...
			pos[1] + (FP.Lbl_Pos_X or 0),
			pos[2] + (FP.Lbl_Pos_Y or 0),
			FP.Lbl_Clr or getClr(r.ImGui_Col_Text()),
			FP.CustomLbl or FP.Name
		)
	end
	...
	if Lbl_Pos == "Top" then
		r.ImGui_DrawList_AddTextEx(
			draw_list,
			_G[Font],
			FX[FxGUID][Fx_P].FontSize or Knob_DefaultFontSize,
			CenteredLblPos or pos[1],
			BtnT - line_height + item_inner_spacing[2],
			FP.Lbl_Clr or 0xffffffff,
	...
	if Style == "Pro C" then
		...
		local txtX = center[1] - TextW / 2
		local txtY = pos[2] + radius_outer * 2 + item_inner_spacing[2]
		...
		local function AutoBtn(Label, offset, Rect_offset)
			if labeltoShow == Label then
				...
				if MouseX > txtX and MouseX < txtX + TextW and MouseY > txtY - 4 and MouseY < txtY + 10 then
					r.ImGui_DrawList_AddRectFilled(
						draw_list,
						txtX + (Rect_offset or 0),
						txtY,
						txtX + TextW + (Rect_offset or 0),
						txtY + 10,
						0x99999955,
						3
					)
	...
	elseif Style == "FX Layering" then
		...
		r.ImGui_DrawList_AddLine(
			...
			center[1] + angle_cos * (radius_outer - 2),
			center[2] + angle_sin * (radius_outer - 2),
			FX[FxGUID][Fx_P].GrbClr or Clr_SldrGrab,
			2.0
		)
	...
	elseif Style == "Analog 1" then
		...
		r.ImGui_DrawList_AddLine(
			...
			center[2] + angle_sin * (radius_outer - 2),
			FX[FxGUID][Fx_P].GrbClr or Clr_SldrGrab,
			FX[FxGUID][Fx_P].Value_Thick or 2
		)
		...
		if FX[FxGUID].MorphA[P_Num] and FX[FxGUID].MorphB[P_Num] then
			...
			if FP.TweakingAB_Val == P_Num and not MorphingMenuOpen then
				...
				if IsLBtnHeld then
					...
					Y_A = MsY - 15
					offsetA = -10
					if MsX < B + 5 and MsX > B - 14 then
						offsetB = 10
					end
				elseif IsRBtnHeld then
					X_B = MsX
					offsetB = -10
					if MsX < A + 5 and MsX > A - 14 then
						offsetA = 10
					end
				end
	...
	if Lbl_Pos == "Bottom" then --Write Bottom Label
		local T = pos[2] + radius_outer * 2 + item_inner_spacing[2]
		local R = pos[1] + radius_outer * 2
		local L = pos[1]
	...
	if V_Pos ~= "None" and V_Pos then
		...
		if FX[FxGUID][Fx_P].V_Round then
			FormatPV = RoundPrmV(FormatPV, FX[FxGUID][Fx_P].V_Round)
		end
		...
		if V_Pos ~= "Free" then
			r.ImGui_DrawList_AddTextEx(
				...
				FX[FxGUID][Fx_P].V_FontSize or Knob_DefaultFontSize,
				CenteredVPos,
				pos[2] + radius_outer * 2 + item_inner_spacing[2] - (Y_Offset or 0),
				FX[FxGUID][Fx_P].V_Clr or 0xffffffff,
				FormatPV,
				(Radius or 20) * 2
			)
	...
	if Lbl_Pos == "Within" and Style == "FX Layering" then
		...
		r.ImGui_DrawList_AddTextEx(
			...
			CenteredVPos,
			pos[2] + radius_outer * 2 + item_inner_spacing[2] - (Y_Offset or 0),
			FX[FxGUID][Fx_P].V_Clr or 0xffffff88,
			labeltoShow,
			(Radius or 20) * 2
		)
	end
	...
	if Trk.Prm.Assign and F_Tp == Trk.Prm.Assign and AssigningMacro then
		...
		if FP.ModAMT[M] + p_value > 1 then
			FP.ModAMT[M] = 1 - p_value
		end
		if FP.ModAMT[M] + p_value < 0 then
			FP.ModAMT[M] = -p_value
		end
		...
		r.SetProjExtState(
			0,
			"FX Devices",
			"Param -" .. Trk.Prm.Assign .. "Macro - " .. AssigningMacro .. FxGUID,
			FP.ModAMT[M]
		)
	end
...
function Add_WetDryKnob(ctx, label, labeltoShow, p_value, v_min, v_max, FX_Idx)
	...
	reaper.ImGui_InvisibleButton(
		...
		label,
		radius_outer * 2,
		radius_outer * 2 + line_height - 10 + item_inner_spacing[2]
	)
	...
	if FX[FxGUID].DeltaP_V ~= 1 then
		...
		r.ImGui_DrawList_AddTriangleFilled(
			draw_list,
			center[1] - radius_outer,
			center[2] + radius_outer,
			center[1],
			center[2] - radius_outer,
			center[1] + radius_outer,
			center[2] + radius_outer,
		...
		r.ImGui_DrawList_AddText(
			draw_list,
			center[1] - radius_outer / 2 + 1,
			center[2] - radius_outer / 2,
			0xffffffff,
			"S"
		)
	end
...
function AddSlider(
	...
	if FX[FxGUID][Fx_P].Name then
		...
		if FX[FxGUID].Morph_Value_Edit or Mods == Alt + Ctrl or LBtnDC then
			r.ImGui_BeginDisabled(ctx)
		end
		...
		if not Sldr_Width or Sldr_Width == "" then
			Sldr_Width = FX.Def_Sldr_W[FxGUID] or Def_Sldr_W or 160
		end
		r.ImGui_SetNextItemWidth(ctx, Sldr_Width)
		r.ImGui_BeginGroup(ctx)

		if SliderStyle == "Pro C Thresh" then
			r.ImGui_PushStyleColor(ctx, r.ImGui_Col_FrameBg(), 0x99999900)
			r.ImGui_PushStyleColor(ctx, r.ImGui_Col_FrameBgActive(), 0x99999922)
		...
		if Vertical == "Vert" then
			...
			if FP.V_Pos == "Top" then
				...
				r.ImGui_SetCursorPosX(ctx, CurX - w / 2 + Sldr_Width / 2)
				if Get then
					MyText(Param_Value, _G[V_Font], r.ImGui_GetColor(ctx, r.ImGui_Col_Text()))
				end
			end
		end
		...
		if FX[FxGUID][Fx_P].V_Round then
			Format_P_V = RoundPrmV(StrToNum(Format_P_V), FX[FxGUID][Fx_P].V_Round)
		end
		...
		if FP.V_Pos == "Free" then
			...
			r.ImGui_DrawList_AddTextEx(
				draw_list,
				Arial,
				FP.V_FontSize or Knob_DefaultFontSize,
				Ox + Sldr_Width - TextW + (FP.V_Pos_X or 0),
				Oy + (FP.V_Pos_Y or 0),
				FP.V_Clr or 0xffffffff,
				Format_P_V
			)
		end
...
function AddDrag(
	...
	r.ImGui_BeginGroup(ctx)
	local BgClr
	if SliderStyle == "Pro C" or SliderStyle == "Pro C Lookahead" then
		BgClr = 0x55555544
	end
	...
	elseif Lbl_Pos == "Free" then
		r.ImGui_DrawList_AddTextEx(
			WDL,
			Arial,
			FP.FontSize or Knob_DefaultFontSize,
			pos[1] + (FP.Lbl_Pos_X or 0),
			pos[2] + (FP.Lbl_Pos_Y or 0),
			FP.V_Clr or 0xffffffff,
			labeltoShow
	...
	local flag
	if AllowInput == "NoInput" then
		flag = r.ImGui_SliderFlags_NoInput()
	end
	...
	r.ImGui_PopStyleColor(ctx, 3)
	local PosL, PosT = r.ImGui_GetItemRectMin(ctx)
	...
	if not SliderStyle then
		...
		elseif DragDir == "Left-Right" then
			local L = math.min(PosL + (PosR - PosL) / 2, PosL + SldrGrbPos)
			local R = math.max(PosL + (PosR - PosL) / 2, PosL + SldrGrbPos)
			if is_active then
	...
	if
		...
		and (FX[FxGUID][Fx_P].V_Pos == "None" or Style == "Pro C" or Style == "Pro C Lookahead")
	then
	...
	if
		LT_ParamNum == P_Num
		and focusedFXState == 1
		and LT_FXGUID == FxGUID
		and FX[FxGUID][Fx_P].Name
		and not FP.WhichCC
	then
	...
	if (V_Pos == "Within" or Lbl_Pos == "Left") and V_Pos ~= "None" and V_Pos ~= "Free" and V_Pos then
		r.ImGui_DrawList_AddTextEx(
			...
			SldrL + W / 2 - TextW / 2,
			SldrT + H / 2 - 5,
			FX[FxGUID][Fx_P].V_Clr or txtclr,
			Format_P_V
		)
	...
	if not Lbl_Pos or Lbl_Pos == "Bottom" then
		...
		if item_inner_spacing then
			if item_inner_spacing < 0 then
				r.ImGui_SetCursorPosY(ctx, r.ImGui_GetCursorPosY(ctx) + item_inner_spacing)
			end
		end
		if Disable == "Disabled" then
			MyText(labeltoShow, _G[Font], TxtDisClr)
			if not string.find(FX.Win_Name_S[FX_Idx] or "", "Pro%-C 2") then
		...
		if
			Style ~= "Pro C Lookahead"
			and Style ~= "Pro C"
			and (not FX[FxGUID][Fx_P].V_Pos or FX[FxGUID][Fx_P].V_Pos == "Right")
		then
	...
	if Lbl_Clickable == "Lbl_Clickable" then
		...
		if HvrText then
			TextL, TextY = r.ImGui_GetItemRectMin(ctx)
			TxtSize = r.ImGui_CalcTextSize(ctx, labeltoShow)
		...
		if ClickText then
			if Style == "Pro C Lookahead" then
				local OnOff
				if OnOff == nil then
					OnOff = r.TrackFX_GetParamNormalized(LT_Track, FX_Idx, 41)
				end
				if OnOff == 1 then
	...
	if FX[FxGUID].Morph_Value_Edit or is_hovered and Mods == Alt + Ctrl then
		r.ImGui_EndDisabled(ctx)
	end
...
function MakeModulationPossible(FxGUID, Fx_P, FX_Idx, P_Num, p_value, Sldr_Width, Type)
	...
	if --[[Link CC back when mouse is up]]
		Tweaking == P_Num .. FxGUID and IsLBtnHeld == false
	then
	...
	if --[[Assign Mod]]
		AssigningMacro and r.ImGui_IsItemClicked(ctx, 1)
	then
		...
		elseif
			Trk.Prm.WhichMcros[CC .. TrkID] and not string.find(
				Trk.Prm.WhichMcros[CC .. TrkID],
				tostring(AssigningMacro)
			)
		then --if there's more than 1 macro assigned, and the assigning macro is new to this param.
			Trk.Prm.WhichMcros[CC .. TrkID] = Trk.Prm.WhichMcros[CC .. TrkID] .. tostring(AssigningMacro)
		end
		r.GetSetMediaTrackInfo_String(
			LT_Track,
			"P_EXT: FX" .. FxGUID .. "Prm" .. Fx_P .. "Linked to which Mods",
			FP.WhichMODs,
			true
		)
	...
	if--[[Right Dragging to adjust Mod Amt]]
		Trk.Prm.Assign
		and FP.WhichCC == Trk.Prm.Assign
		and AssigningMacro
	then
		...
		if not IsLBtnHeld then
			r.gmem_write(4, 1)
		end --tells jsfx that user is changing Macro Mod Amount
		r.gmem_write(1000 * AssigningMacro + Trk.Prm.Assign, FP.ModAMT[M])
		r.ImGui_ResetMouseDragDelta(ctx, 1)
...
function CheckIfLayoutEditHasBeenMade(FxGUID, FX_Name)
	if FX[FxGUID].File then
		...
		if FX.GrbRound[FxGUID] ~= (get_aftr_Equal_Num(Ln[4]) or 0) then
		end
		if FX.Round[FxGUID] ~= (get_aftr_Equal_Num(Ln[3]) or 0) then
		end
		if FX.BgClr[FxGUID] ~= get_aftr_Equal_Num(Ln[5]) then
		end
		if FX.TitleWidth[FxGUID] ~= (get_aftr_Equal_Num(Ln[7]) or 0) then
		end
		if FX.Width[FxGUID] ~= (get_aftr_Equal_Num(Ln[6]) or 0) then
		end
		...
		for Fx_P = 1, #FX[FxGUID] or 0, 1 do
			...
			if
				FP.Name ~= get_aftr_Equal_Num(L(14))
				or FP.Num ~= get_aftr_Equal_Num(L(15))
				or FP.Sldr_W ~= get_aftr_Equal_Num(L(16))
				or FP.Type ~= get_aftr_Equal_(L(17))
				or FP.PosX ~= get_aftr_Equal_Num(L(18))
				or FP.PosY ~= get_aftr_Equal_Num(L(19))
				or FP.Style ~= get_aftr_Equal(L(20))
				or FP.V_FontSize ~= get_aftr_Equal_Num(L(21))
				or FP.CustomLbl ~= get_aftr_Equal_Num(L(22))
				or FP.FontSize ~= get_aftr_Equal_Num(L(23))
				or FP.Sldr_H ~= "1"
				or FP.BgClr ~= "2"
				or FP.GrbClr ~= "3"
				or FP.Lbl_Pos ~= "4"
				...
				or FP.V_Clr ~= "4"
				or FP.DragDir ~= "4"
				or FP.ConditionPrm ~= "4"
			then
				ChangeBeenMade = true
			end
		end

		if FX[FxGUID].AllPrmHasBeenDeleted then
			ChangeBeenMade = true
		end
...
function CheckIfDrawingHasBeenMade(FX_Idx)
	...
	for i, Type in pairs(D.Type) do
		if
			D.L[i] ~= tonumber(r.GetExtState("FX Devices Drawings", "prm " .. i .. "s L pos"))
			or D.R[i] ~= tonumber(r.GetExtState("FX Devices Drawings", "prm " .. i .. "s R Pos"))
			or D.T[i] ~= tonumber(r.GetExtState("FX Devices Drawings", "prm " .. i .. "s T Pos"))
			or D.B[i] ~= tonumber(r.GetExtState("FX Devices Drawings", "prm " .. i .. "s B Pos"))
			or D.Txt[i] ~= tonumber(r.GetExtState("FX Devices Drawings", "prm " .. i .. "s Txt"))
			or D.clr[i] ~= tonumber(r.GetExtState("FX Devices Drawings", "prm " .. i .. "s Clr"))
		then
			ChangeBeenMade = true
		end
	end
...
function ShowStyleEditor()
	...
	if not ctx then
		ctx = r.ImGui_CreateContext("Style Editor 2")
	end
	if not styleEditorIsOpen then
		r.ImGui_SetNextWindowSize(ctx, 500, 800)
	end
	open, OpenStyleEditor = r.ImGui_Begin(
		ctx,
		"FX Devices Style Editor",
		OpenStyleEditor,
		r.ImGui_WindowFlags_TopMost() + r.ImGui_WindowFlags_NoCollapse() + r.ImGui_WindowFlags_NoDocking() --[[ +r.ImGui_WindowFlags_AlwaysAutoResize() ]]
	)
	...
	if open then
		...
		local FrameRounding, GrabRounding = r.ImGui_StyleVar_FrameRounding(), r.ImGui_StyleVar_GrabRounding()
		...
		rv, app.style_editor.style.vars[Alpha] =
			r.ImGui_DragDouble(ctx, "Global Alpha", app.style_editor.style.vars[Alpha], 0.005, 0.20, 1.0, "%.2f") -- Not exposing zero here so user doesn't "lose" the UI (zero alpha clips all widgets). But application code could have a toggle to switch between zero and non-zero.
		r.ImGui_SameLine(ctx)
		rv, app.style_editor.style.vars[DisabledAlpha] = r.ImGui_DragDouble(
			ctx,
			"Disabled Alpha",
			app.style_editor.style.vars[DisabledAlpha],
			0.005,
			0.0,
			1.0,
			"%.2f"
		...
		if
			r.ImGui_TextFilter_Draw(
				app.style_editor.colors.filter.inst,
				ctx,
				"Filter colors",
				r.ImGui_GetFontSize(ctx) * 16
			)
		then
		...
		if
			r.ImGui_BeginChild(ctx, "##colors", 0, 0, true, r.ImGui_WindowFlags_AlwaysVerticalScrollbar() | 
		...
		then
			...
			for i, v in pairs(CustomColors) do
				if r.ImGui_TextFilter_PassFilter(app.style_editor.colors.filter.inst, v) then
					rv, _G[v] = r.ImGui_ColorEdit4(
						ctx,
						"##" .. v,
						_G[v] or CustomColorsDefault[v],
						r.ImGui_ColorEditFlags_AlphaBar() | app.style_editor.colors.alpha_flags
					)
					r.ImGui_SameLine(ctx, 0.0, inner_spacing)
...
function StoreNewParam(FxGUID, P_Name, P_Num, FX_Num, IsDeletable, AddingFromExtState, Fx_P, FX_Idx, TrkID)
	...
	if AddingFromExtState == "AddingFromExtState" then
		...
		local rv, step, smallstep, largestep, istoggle =
			r.TrackFX_GetParameterStepSizes(LT_Track, LT_FX_Number, LT_ParamNum)
		if rv then --[[ if the param is a switch ]]
		end
	...
	if LT_Track then
		for FX_Idx = 0, Sel_Track_FX_Count - 1, 1 do
			...
			if FxGUID then
				...
				if --[[ r.GetExtState('FX Devices - '..FX_Name, 'Param Instance') ~= ''  ]]
					PrmInst
				then
					local Ct = Content
					...
					if PrmCount then
						for Fx_P = 1, PrmCount or 0, 1 do
							...
							if FP.ConditionPrm then
								...
								FP.ConditionPrm_V_Norm =
									RecallIntoTable(Ct, Fx_P .. ". Condition Param Norm = |1=", Fx_P, "Num")
							end
							for i = 2, 5, 1 do
								FP["ConditionPrm_V" .. i] =
									RecallIntoTable(Ct, Fx_P .. ". Condition Param" .. i .. " = %d+|1=", Fx_P, nil)
								FP["ConditionPrm_V_Norm" .. i] =
									RecallIntoTable(Ct, Fx_P .. ". Condition Param Norm" .. i .. " = |1=", Fx_P, "Num")
							end
...
for Track_Idx = 0, NumOfTotalTracks - 1, 1 do
	...
	for i = 1, 8, 1 do -- for every modulator
		Trk[TrkID].Mod[i] = {}
		Trk[TrkID].Mod[i].ATK =
			tonumber(select(2, r.GetSetMediaTrackInfo_String(Track, "P_EXT: Macro " .. i .. " Atk", "", false)))
		Trk[TrkID].Mod[i].REL =
			tonumber(select(2, r.GetSetMediaTrackInfo_String(Track, "P_EXT: Macro " .. i .. " Rel", "", false)))
		Trk[TrkID].SEQL[i] =
			tonumber(select(2, r.GetSetMediaTrackInfo_String(Track, "P_EXT: Macro " .. i .. " SEQ Length", "", false)))
		Trk[TrkID].SEQ_Dnom[i] = tonumber(
			select(2, r.GetSetMediaTrackInfo_String(Track, "P_EXT: Macro " .. i .. " SEQ Denominator", "", false))
		)
		...
		if Trk[TrkID].SEQL[i] then
			for St = 1, Trk[TrkID].SEQL[i], 1 do
				Trk[TrkID].Mod[i].SEQ[St] = tonumber(
					select(
						2,
						r.GetSetMediaTrackInfo_String(
							Track,
							"P_EXT: Macro " .. i .. " SEQ Step = " .. St .. " Val",
							"",
							false
						)
					)
	...
	for CC = 1, Trk[TrkID].ModPrmInst or 0, 1 do
		_, Trk.Prm.WhichMcros[CC .. TrkID] =
			r.GetSetMediaTrackInfo_String(Track, "P_EXT: CC Linked to which Modulation" .. CC, "", false)
	end
	...
	if Trk[TrkID].PreFX == {} then
		Trk[TrkID].PreFX = nil
	end
	for P = 1, Trk.Prm.Inst[TrkID] or 0, 1 do
		_, Trk.Prm.Num[P .. TrkID] = r.GetProjExtState(0, "FX Devices", "Track" .. TrkID .. " P =" .. P)
		_, Trk.Prm.WhichMcros[P .. TrkID] =
			r.GetProjExtState(0, "FX Devices", "Prm" .. P .. "Has Which Macro Assigned, TrkID =" .. TrkID)
		if Trk.Prm.WhichMcros[P .. TrkID] == "" then
			Trk.Prm.WhichMcros[P .. TrkID] = nil
		end
	...
	for FX_Idx = 0, FXCount - 1, 1 do --repeat as many times as fx instances
		...
		if DefaultSldr_W ~= "" then
			FX.Def_Sldr_W[FxGUID] = DefaultSldr_W
		end
		local _, Def_Type = r.GetProjExtState(0, "FX Devices", "Default Param type for FX:" .. FxGUID)
		if Def_Type ~= "" then
			FX.Def_Type[FxGUID] = Def_Type
		end
		...
		if FxGUID ~= nil then
			...
			_, FX[FxGUID].ContainerTitle =
				r.GetProjExtState(0, "FX Devices - ", "FX" .. FxGUID .. "FX Layer Container Title ")
			if FX[FxGUID].ContainerTitle == "" then
				FX[FxGUID].ContainerTitle = nil
			end
			...
			if FX.InLyr[FxGUID] == "" then
				FX.InLyr[FxGUID] = nil
			end
			FX[FxGUID].Morph_ID =
				tonumber(select(2, r.GetSetMediaTrackInfo_String(Track, "P_EXT: FXs Morph_ID" .. FxGUID, "", false)))
			_, FX[FxGUID].Unlink =
				r.GetSetMediaTrackInfo_String(Track, "P_EXT: FXs Morph_ID" .. FxGUID .. "Unlink", "", false)
			if FX[FxGUID].Unlink == "Unlink" then
				FX[FxGUID].Unlink = true
			elseif FX[FxGUID].Unlink == "" then
				FX[FxGUID].Unlink = nil
			end
			...
			if rv then
				FX[FxGUID].ProC_ID = tonumber(ProC_ID)
			end
			...
			for Fx_P = 1, #FX[FxGUID] or 0, 1 do
				...
				_, FX.Prm.ToTrkPrm[FxGUID .. Fx_P] =
					r.GetProjExtState(0, "FX Devices", "FX" .. FxGUID .. "Prm" .. Fx_P .. "to Trk Prm")
				FX.Prm.ToTrkPrm[FxGUID .. Fx_P] = tonumber(FX.Prm.ToTrkPrm[FxGUID .. Fx_P])

				local F_Tp = FX.Prm.ToTrkPrm[FxGUID .. Fx_P]

				_G[ParamX_Value] = FX[FxGUID][Fx_P].V or 0
				FX[FxGUID][Fx_P].WhichCC = tonumber(
					select(
						2,
						r.GetSetMediaTrackInfo_String(
							Track,
							"P_EXT: FX" .. FxGUID .. "WhichCC" .. (FX[FxGUID][Fx_P].Num or 0),
							"",
							false
						)
					)
				)
				_, FX[FxGUID][Fx_P].WhichMODs = r.GetSetMediaTrackInfo_String(
					Track,
					"P_EXT: FX" .. FxGUID .. "Prm" .. Fx_P .. "Linked to which Mods",
					"",
					false
				)
				if FX[FxGUID][Fx_P].WhichMODs == "" then
				...
				local HasModAmt
				for m = 1, 8, 1 do
					FX[FxGUID][Fx_P].ModAMT[m] = tonumber(
						select(
							2,
							r.GetSetMediaTrackInfo_String(
								Track,
					...
					if FX[FxGUID][Fx_P].ModAMT[m] then
						HasModAmt = true
					end

					Trk[TrkID].Mod = Trk[TrkID].Mod or {}
					Trk[TrkID].Mod[m] = Trk[TrkID].Mod[m] or {}
					Trk[TrkID].Mod[m].Val = tonumber(
						select(2, r.GetProjExtState(0, "FX Devices", "Macro" .. m .. "Value of Track" .. TrkID))
					)
				...
				if not HasModAmt then
					FX[FxGUID][Fx_P].ModAMT = nil
				end
			end
			...
			if r.GetSetMediaTrackInfo_String(Track, "P_EXT: FX Morph A" .. "1" .. FxGUID, "", false) then
				...
				for i = 0, PrmCount - 4, 1 do
					_, FX[FxGUID].MorphA[i] =
						r.GetSetMediaTrackInfo_String(Track, "P_EXT: FX Morph A" .. i .. FxGUID, "", false)
					FX[FxGUID].MorphA[i] = tonumber(FX[FxGUID].MorphA[i])
					_, FX[FxGUID].MorphB[i] =
						r.GetSetMediaTrackInfo_String(Track, "P_EXT: FX Morph B" .. i .. FxGUID, "", false)
					FX[FxGUID].MorphB[i] = tonumber(FX[FxGUID].MorphB[i])
				end

				_, FX[FxGUID].MorphA_Name =
					r.GetSetMediaTrackInfo_String(Track, "P_EXT: FX Morph A" .. FxGUID .. "Preset Name", "", false)
				if FX[FxGUID].MorphA_Name == "" then
					FX[FxGUID].MorphA_Name = nil
				end
				_, FX[FxGUID].MorphB_Name =
					r.GetSetMediaTrackInfo_String(Track, "P_EXT: FX Morph B" .. FxGUID .. "Preset Name", "", false)
				if FX[FxGUID].MorphB_Name == "" then
					FX[FxGUID].MorphB_Name = nil
				end
			end
		end
		...
		if string.find(FX_Name, "FXD %(Mix%)RackMixer") or string.find(FX_Name, "FXRack") then
			...
			for i = 1, 8, 1 do
				_, FX[FXGUIDofRackMixer].LyrID[i] =
					r.GetProjExtState(0, "FX Devices", "FX" .. FXGUIDofRackMixer .. "Layer ID " .. i)
				_, FX[FXGUIDofRackMixer].LyrTitle[i] =
					r.GetProjExtState(0, "FX Devices - ", "FX" .. FXGUIDofRackMixer .. "Layer Title " .. i)
				if FX[FXGUIDofRackMixer].LyrTitle[i] == "" then
					FX[FXGUIDofRackMixer].LyrTitle[i] = nil
				end
			...
			if Lyr.FX_Ins[FXGUIDofRackMixer] == "" then
				Lyr.FX_Ins[FXGUIDofRackMixer] = nil
			end
			Lyr.FX_Ins[FXGUIDofRackMixer] = tonumber(Lyr.FX_Ins[FXGUIDofRackMixer])
		elseif FX_Name:find("FXD Saike BandSplitter") then
			FX[FxGUID].BandSplitID =
				tonumber(select(2, r.GetSetMediaTrackInfo_String(Track, "P_EXT: BandSplitterID" .. FxGUID, "", false)))
			_, FX[FxGUID].AttachToJoiner =
				r.GetSetMediaTrackInfo_String(Track, "P_EXT: Splitter's Joiner FxID " .. FxGUID, "", false)

			for FX_Idx = 0, FXCount - 1, 1 do --repeat as many times as fx instances
				--Restore Band Split
				local FxID = r.TrackFX_GetFXGUID(Track, FX_Idx)
				if
					select(2, r.GetSetMediaTrackInfo_String(Track, "P_EXT: FX is in which BS" .. FxID, "", false))
					== FxGUID
				then
					--local _, Guid_FX_In_BS = r.GetSetMediaTrackInfo_String(LT_Track,'P_EXT: FX is in which BS'..FxID, '', false  )
					FX[FxID] = FX[FxID] or {}
					FX[FxID].InWhichBand = tonumber(
						select(2, r.GetSetMediaTrackInfo_String(Track, "P_EXT: FX is in which Band" .. FxID, "", false))
					)
	...
	for m = 1, 8, 1 do
		_, Trk[TrkID].Mod[m].Name =
			r.GetSetMediaTrackInfo_String(Track, "P_EXT: Macro" .. m .. "s Name" .. TrkID, "", false)
		if Trk[TrkID].Mod[m].Name == "" then
			Trk[TrkID].Mod[m].Name = nil
		end
		_, Trk[TrkID].Mod[m].Type = r.GetSetMediaTrackInfo_String(Track, "P_EXT: Mod" .. m .. "Type", "", false)
		if Trk[TrkID].Mod[m].Type == "" then
			Trk[TrkID].Mod[m].Type = nil
		end
	end
end
...
function loop()
	...
	if LT_Track then
		TrkClr = r.ImGui_ColorConvertNative(r.GetTrackColor(LT_Track))
	end
	...
	function DeleteFX(FX_Idx)
		...
		r.GetSetMediaTrackInfo_String(
			LT_Track,
			"P_EXT: PreFX " .. (tablefind(Trk[TrkID].PreFX, FXGUID[FX_Idx]) or ""),
			"",
			true
		)
				...
				for Fx_P = 1, #FX[FxGUID] or 0, 1 do
					if TrkID then
						if not FX[FxGUID][Fx_P].WhichMODs then
							FX[FxGUID][Fx_P].V =
								r.TrackFX_GetParamNormalized(LT_Track, FX_Idx, FX[FxGUID][Fx_P].Num or 0)
						end
					end
				end
			end
	...
	if visible then
		...
		if LT_Track == nil then
			...
			for i, v in ipairs(AddFX.Name) do
				...
				if v:find("FXD Band Joiner") then
					...
					r.GetSetMediaTrackInfo_String(
						LT_Track,
						"P_EXT: Splitter's Joiner FxID " .. SplittrID,
						JoinerID,
						true
					)
			...
			if r.ImGui_Button(ctx, "Record Last Touch") then
				...
				if F then
					...
					if not RptPrmFound and LT_FXGUID then
						StoreNewParam(LT_FXGUID, LT_ParamName, LT_ParamNum, LT_FXNum, true--[[ , nil, #F+1  ]])
					end
				end
			end
			...
			if FX.LayEdit then
				...
				if #LE.Sel_Items > 1 then
					...
					if r.ImGui_Button(ctx, "Align Y-Axis") then
						for i, v in ipairs(LE.Sel_Items) do
							FX[FxGUID][v].PosX = FX[FxGUID][LE.Sel_Items[1]].PosX
						end
					elseif r.ImGui_Button(ctx, "Align X-Axis") then
						for i, v in ipairs(LE.Sel_Items) do
							FX[FxGUID][v].PosY = FX[FxGUID][LE.Sel_Items[1]].PosY
						end
					end
				end
				...
				if LT_FXGUID and type(F) == "table" then
					for i, v in ipairs(F) do
						...
						if F[i].Num == LT_ParamNum then
							RptPrmFound = true
							TryingToAddExistingPrm_Cont = i .. LT_FXGUID
							TryingToAddExistingPrm = nil
							TimeNow = r.time_precise()
						end
					end
				...
				elseif m == 2 or m == 4 or m == 6 or m == 8 or m == 10 or m == 12 or m == 14 or m == 16 then
					...
					if Trk[TrkID].Mod[m / 2] then
						if Trk[TrkID].Mod[m / 2].Type == "Step" then
							weight, flag = 0, r.ImGui_TableColumnFlags_WidthFixed()
						end
					end
			...
			for i, v in ipairs(MacroNums) do --Do 8 Times
				...
				if Trk[TrkID].Mod[i].Type == "Macro" then
					...
					if AssigningMacro == i then
						r.ImGui_PopStyleColor(ctx, PopColorTime)
					end
				...
				elseif Trk[TrkID].Mod[i].Type == "Step" then
					...
					function openSEQWin(Track, i)
						...
						then
							...
							Trk[TrkID].SEQL = Trk[TrkID].SEQL or {}
							rv, Trk[TrkID].SEQL[i] = r.ImGui_SliderInt(
								ctx,
								"##" .. "Macro" .. i .. "SEQ Length",
								Trk[TrkID].SEQL[i] or SEQ_Default_Num_of_Steps,
								2,
								64
							)
							if r.ImGui_IsItemActive(ctx) then
							...
							for St = 1, Trk[TrkID].SEQL[i] or SEQ_Default_Num_of_Steps, 1 do
								...
								if
									IsLBtnHeld
									and r.ImGui_IsMouseHoveringRect(ctx, L, T, R, B)
									and not SmallSEQActive
								then
								...
								if
									r.ImGui_IsItemHovered(ctx, r.ImGui_HoveredFlags_RectOnly()) and not SmallSEQActive
								then
									FillClr = 0xffffff22
									Clr = Change_Clr_A(EightColors.Bright_HighSat[i], -0.3)
								end
								...
								r.ImGui_DrawList_AddRectFilled(
									...
									T + StepSEQ_H,
									L + StepSEQ_W - 1,
									math.max(B - StepSEQ_H * (S[St] or 0), T),
									Clr
								)
				...
				if r.ImGui_BeginPopup(ctx, "Macro" .. i .. "Menu") then
					...
					elseif r.ImGui_Selectable(ctx, "Set Type to Step Sequencer") then
						...
						r.GetSetMediaTrackInfo_String(
							LT_Track,
							"P_EXT: Macro " .. i .. " SEQ Denominator",
							Trk[TrkID].SEQ_Dnom[i] or SEQ_Default_Denom,
							true
						)

						if I.Name == "Env " .. i or I.Name == "Macro " .. i then
							I.Name = "Step " .. i
						end
					end
					r.ImGui_EndPopup(ctx)
			...
			if not FX_Dvs_BgDL then
				FX_Dvs_BgDL = r.ImGui_GetWindowDrawList(ctx)
			end
			...
			function AddSpaceBtwnFXs(
				...
				if FX_Idx == 1 and r.TrackFX_AddByName(LT_Track, "FXD Macros", 0, 0) ~= -1 then
					FX_Idx = FX_Idx - 1
				else
					FX_Idx = FX_Idx
				end
				...
				if SpaceIsBeforeRackMixer == "SpcInPost" or SpaceIsBeforeRackMixer == "SpcInPost 1st spc" then
					SpcInPost = true
					if PostFX_LastSpc == 30 then
						Dvdr.Spc_Hover[TblIdxForSpace] = 30
					end
				end
				...
				if not Hide then
					if
						r.ImGui_BeginChildFrame(
							ctx,
							"##SpaceBetweenWindows"
								.. FX_Idx
								.. tostring(SpaceIsBeforeRackMixer)
								.. "Last SPC in Rack = "
								.. tostring(AddLastSPCinRack),
							10
								+ Dvdr.Width[TblIdxForSpace]
								+ (Dvdr.Spc_Hover[TblIdxForSpace] or 0)
								+ (AdditionalWidth or 0),
							220,
							r.ImGui_WindowFlags_NoScrollbar()
								| r.ImGui_WindowFlags_NoScrollWithMouse()
								| r.ImGui_WindowFlags_NoNavFocus()
					...
					then
						...
						if
							HoverOnWindow == true
							and Dragging_TrueUntilMouseUp ~= true
							and DragDroppingFX ~= true
							and AssignWhichParam == nil
							and Is_ParamSliders_Active ~= true
							and Wet.ActiveAny ~= true
							and Knob_Active ~= true
							and not Dvdr.JustDroppedFX
							and LBtn_MousdDownDuration < 0.2
						then
						...
						if r.ImGui_BeginPopup(ctx, "Btwn FX Windows" .. FX_Idx) then
							...
							if r.ImGui_Selectable(ctx, "Add FX Layering") then
								...
								ChanSplitr =
									r.TrackFX_AddByName(LT_Track, "FXD Split to 32 Channels", 0, -1000 - FX_Idx)
				...
				function MoveFX(DragFX_ID, FX_Idx, isMove, AddLastSpace)
					...
					if not DontMove then
						if FX_Idx ~= RepeatTimeForWindows and SpaceIsBeforeRackMixer ~= "End of PreFX" then
							...
							table.insert(MovFX.FromPos, DragFX_ID)
						elseif
							FX_Idx == RepeatTimeForWindows and AddLastSpace == "LastSpc"
							or SpaceIsBeforeRackMixer == "End of PreFX"
						then
							local offset

							if Trk[TrkID].PostFX[1] then
								offset = #Trk[TrkID].PostFX
							end
				...
				local function AddFX_Sexan(Dest)
					...
					if dropped then
						...
						--if in layer
						if
							FX.InLyr[FXGUID_To_Check_If_InLayer] == FXGUID_RackMixer
								and SpaceIsBeforeRackMixer == false
							or AddLastSPCinRack == true
						then
							DropFXtoLayerNoMove(FXGUID_RackMixer, LyrID, FX_Idx)
						end
				...
				if
					FX.InLyr[FXGUID_To_Check_If_InLayer] == FXGUID_RackMixer and SpaceIsBeforeRackMixer == false
					or AddLastSPCinRack == true
				then
					...
					if
						DragFX_ID == FX_Idx
						or DragFX_ID == FX_Idx - 1 and FX.InLyr[FXGUID_of_DraggingFX] == FXGUID[FX_Idx]
					then
					...
					if r.ImGui_BeginDragDropTarget(ctx) then
						if Payload_Type == "FX_Drag" then
							...
							if
								...
								and (not SpcIsInPre or SpaceIsBeforeRackMixer == "End of PreFX")
							then
								allowDropNext = true
							end
							if
								tablefind(Trk[TrkID].PostFX, FXGUID[DragFX_ID])
								and (not SpcInPost or AddLastSpace == "LastSpc")
							then
								allowDropNext = true
							...
							if not FX[FXGUID[DragFX_ID]].InWhichBand and SpaceIsBeforeRackMixer == "SpcInBS" then
								allowDropNext = true
							end
							...
							if
								(DragFX_ID == FX_Idx or DragFX_ID == FX_Idx - 1)
									and SpaceIsBeforeRackMixer ~= true
									and FX.InLyr[FXGUID[DragFX_ID]] == nil
									and not SpcInPost
									and not allowDropNext
								or (Trk[TrkID].PreFX[#Trk[TrkID].PreFX] == FXGUID[DragFX_ID] and SpaceIsBeforeRackMixer == "End of PreFX")
								or DontAllowDrop
							then
								...
								if dropped and Mods == 0 then
									...
									if
										FX.Win_Name_S[payload]:find("Pro%-Q 3")
										and not tablefind(Trk[TrkID].PostFX, FXGUID[payload])
										and not SpcInPost
										and not SpcIsInPre
									...
									if Lyr.FX_Ins[FX.InLyr[FXGUID_To_Check_If_InLayer]] ~= nil then
										Lyr.FX_Ins[FX.InLyr[FXGUID_To_Check_If_InLayer]] = Lyr.FX_Ins[FX.InLyr[FXGUID_To_Check_If_InLayer]]
											- 1
									end
									r.SetProjExtState(
										0,
						...
						elseif Payload_Type == "FX Layer Repositioning" then -- FX Layer Repositioning
							...
							if
								(DragFX_ID - (math.max(lyrFxInst, 1)) <= FX_Idx and FX_Idx <= DragFX_ID + 1)
						...
						elseif Payload_Type == "BS_Drag" then
							...
							if
								SpaceIsBeforeRackMixer == "SpcInBS"
								or FX_Idx == Pl
								or Pl + (#FX[FXGUID[Pl]].FXsInBS or 0) + 2 == FX_Idx
							then
								Dvdr.Width[TblIdxForSpace] = 0
							else
				...
				elseif Payload_Type == "BS_Drag" then
					...
					elseif Payload > FX_Idx and FX_Idx ~= RepeatTimeForWindows then
						reaper.TrackFX_CopyToTrack(
							...
							LT_Track,
							FX_Idx + #FX[FXGUID[Payload]].FXsInBS + 1,
							true
						)
					end
				end
			...
			if Trk[TrkID].PreFX[1] then
				...
				if r.ImGui_IsItemClicked(ctx, 1) then
					if Trk[TrkID].PreFX_Hide then
						Trk[TrkID].PreFX_Hide = false
					else
						Trk[TrkID].PreFX_Hide = true
					end
				end
			end
			...
			if MacroPos ~= -1 and MacroPos ~= 0 then -- if macro exists on track, and Macro is not the 1st fx
				if FX.Win_Name[0] ~= "JS: FXD Macros" then
					r.TrackFX_CopyToTrack(LT_Track, MacroPos, LT_Track, 0, true)
				end -- move it to 1st slot
			end

			if MacroPos ~= -1 or ReSpectrumPos == 0 then
				offset = 0
			else
				offset = 1
			end -- if no Macros is found
			...
			if Trk[TrkID].PreFX[1] and Trk[TrkID].PostFX[1] and not Trk[TrkID].PostFX_Hide then
				spaceIfPreFX = 20
			end
			if Wheel_V ~= 0 and not DisableScroll then
				r.ImGui_SetNextWindowScroll(ctx, -CursorStartX + Wheel_V * 10, 0)
			end
			...
			if
				r.ImGui_BeginChild(
					ctx,
					"fx devices",
					MaxX - (PostFX_Width or 0) - spaceIfPreFX,
					240,
					nil,
					r.ImGui_WindowFlags_HorizontalScrollbar() + FX_DeviceWindow_NoScroll
				)
			then
				------------------------------------------------------
				...
				for FX_Idx = 0, Sel_Track_FX_Count - 1, 1 do
					...
					if not tablefind(Trk[TrkID].PostFX, FxGUID) and FXGUID[FX_Idx] ~= FXGUID[FX_Idx - 1] then
						if
							FX.InLyr[FXGUID_To_Check_If_InLayer] == nil --not in layer
							...
							and not FX[FxGUID].InWhichBand --[[Not in Band Split]]
						then
							local Idx = FX_Idx
							if FX_Idx == 1 then
								local Nm = FX.Win_Name[0]
								if Nm == "JS: FXD Macros" or FindStringInTable(BlackListFXs, Nm) then
									Idx = 0
								end
							end
							AddSpaceBtwnFXs(Idx)
					...
					function createFXWindow(FX_Idx)
						...
						if
							FXGUID[FX_Idx] ~= FXGUID[FX_Idx - 1] --[[  findDuplicates(FXGUID) ]]
							and FxGUID
						then
							r.ImGui_BeginGroup(ctx)
							...
							if FX[FxGUID].MorphA and not FX[FxGUID].MorphHide then
								...
								function StoreAllPrmVal(AB, DontStoreCurrentVal, LinkCC)
									...
									for i = 0, PrmCount - 4, 1 do
										...
										if AB == "A" then
											...
											if DontStoreCurrentVal ~= "Dont" then
												FX[FxGUID].MorphB[i] = Prm_Val
											end
											if FX[FxGUID].MorphB[i] then
												r.GetSetMediaTrackInfo_String(
													LT_Track,
													"P_EXT: FX Morph B" .. i .. FxGUID,
													FX[FxGUID].MorphB[i],
													true
												)
												if LinkCC then
									...
									if DontStoreCurrentVal ~= "Dont" then
										...
										if rv and AB == "A" then
											...
											r.GetSetMediaTrackInfo_String(
												LT_Track,
												"P_EXT: FX Morph A" .. FxGUID .. "Preset Name",
												presetname,
												true
											)
										elseif rv and AB == "B" then
											...
											r.GetSetMediaTrackInfo_String(
												LT_Track,
												"P_EXT: FX Morph B" .. FxGUID .. "Preset Name",
												presetname,
												true
											)
										end
								...
								if r.ImGui_IsItemActive(ctx) then
									...
									isActive = true
									BgClrA = DefClr_A_Act
									BgClrB = DefClr_B_Act -- shift 0x00RRGGBB to 0xRRGGBB00 then add 0xFF for 100% opacity
								elseif r.ImGui_IsItemHovered(ctx) then
									ifHvr = true
									BgClrA = DefClr_A_Hvr
									BgClrB = DefClr_B_Hvr
								...
								if --[[Ctrl + R click]]
									r.ImGui_IsItemClicked(ctx, 1) and Mods == Ctrl
								then
								...
								if isActive then
									...
									FX[FxGUID].MorphAB_Sldr =
										SetMinMax((FX[FxGUID].MorphAB_Sldr or 0) + v / (DrgSpdMod or 2), 0, 100)
								...
								if r.ImGui_BeginPopup(ctx, "Morphing menu" .. FX_Idx) then
									...
									if not FX[FxGUID].Morph_ID or FX[FxGUID].Unlink then
										if r.ImGui_Selectable(ctx, "Automate") then
											...
											if --[[Add Macros JSFX if not found]]
												r.TrackFX_AddByName(LT_Track, "FXD Macros", 0, 0) == -1
												and r.TrackFX_AddByName(LT_Track, "FXD Macros", 0, 0) == -1
											then
												r.gmem_write(1, PM.DIY_TrkID[TrkID]) --gives jsfx a guid when it's being created, this will not change becuase it's in the @init.
												AddMacroJSFX()
											end
											...
											r.GetSetMediaTrackInfo_String(
												LT_Track,
												"P_EXT: FXs Morph_ID" .. FxGUID .. "Unlink",
												"",
												true
											)
								...
								if not FX[FxGUID].MorphB[1] then
									BtnB_TxtClr = r.ImGui_GetStyleColor(ctx, r.ImGui_Col_TextDisabled())
								end
								...
								if r.ImGui_IsItemHovered(ctx) and FX[FxGUID].MorphB_Name then
									HintToolTip(FX[FxGUID].MorphB_Name)
								end
								r.ImGui_PopStyleColor(ctx, 3)
							...
							local PrmCount = tonumber(
								select(2, r.GetProjExtState(0, "FX Devices", "Prm Count" .. FxGUID))
							) or 0
							local Def_Sldr_W = 160
							if FX.Def_Sldr_W[FxGUID] then
								Def_Sldr_W = FX.Def_Sldr_W[FxGUID]
							end

							if
								FX.Def_Type[FxGUID] == "Slider"
								or FX.Def_Type[FxGUID] == "Drag"
								or not FX.Def_Type[FxGUID]
							then
							...
							if not Hide then
								...
								if
									r.ImGui_BeginChild(
										ctx,
										FX_Name .. FX_Idx,
										FX.WidthCollapse[FxGUID] or FX.Width[FXGUID[FX_Idx]] or DefaultWidth,
										220,
										nil,
										r.ImGui_WindowFlags_NoScrollWithMouse() | r.ImGui_WindowFlags_NoScrollbar()
									) and not Hide
								then ----START CHILD WINDOW------
									...
									if Draw.DrawMode[FxGUID] == true then
										...
										if
											r.ImGui_IsMouseHoveringRect(ctx, Win_L, Win_T, Win_R, Win_B)
											and HvringItmSelector == nil
											and not Draw.SelItm
											and Draw.Time == 0
										then
										...
										for i, Type in pairs(D.Type) do
											...
											then
												...
												if IsLBtnClicked and ModifierHeld == Alt then
													...
													table.remove(D.B, i)
													if D.Txt[i] then
														table.remove(D.Txt, i)
													end
													if D.clr[i] then
														table.remove(D.clr, i)
													end
													if r.ImGui_BeginPopup(ctx, "Drawlist Add Text Menu") then
														r.ImGui_CloseCurrentPopup(ctx)
														r.ImGui_EndPopup(ctx)
									...
									if FX.LayEdit == FxGUID and Draw.DrawMode[FxGUID] ~= true then -- Resize FX or title btn
										...
										if LE.ResizingFX == FX_Idx and IsLBtnHeld then
											...
											local MsDragDeltaX, MsDragDeltaY = r.ImGui_GetMouseDragDelta(ctx)
											local Dx, Dy = r.ImGui_GetMouseDelta(ctx)
											if not FX.Width[FXGUID[FX_Idx]] then
												FX.Width[FXGUID[FX_Idx]] = DefaultWidth
											end
											FX.Width[FXGUID[FX_Idx]] = FX.Width[FXGUID[FX_Idx]] + Dx
											LE.BeenEdited = true
										end
										if not IsLBtnHeld then
											LE.ResizingFX = nil
									...
									if FX[FxGUID].TitleClr then
										...
										if not FX[FxGUID].TitleClrHvr then
											FX[FxGUID].TitleClrAct, FX[FxGUID].TitleClrHvr =
												Generate_Active_And_Hvr_CLRs(FX[FxGUID].TitleClr)
										end
										r.ImGui_PushStyleColor(
											ctx,
									...
									if FX[FxGUID].Collapse ~= true then
										...
										if
											...
											and FindStringInTable(SpecialLayoutFXs, FX_Name) == false
										then
									...
									if
										r.ImGui_BeginPopup(
											ctx,
											"Add Parameter" .. FxGUID,
											r.ImGui_WindowFlags_AlwaysVerticalScrollbar()
										)
									then
										AddPrmPopupOpen = FxGUID
										if not PrmFilterTxt then
											AddPrmWin_W, AddPrmWin_H = r.ImGui_GetWindowSize(ctx)
										end
										...
										for i = 1, Ct, 1 do
											...
											if r.ImGui_TextFilter_PassFilter(PrmFilter, P_Name) then
												rv[i], CheckBox[i - 1] =
													r.ImGui_Checkbox(ctx, (i - 1) .. ". " .. P_Name, CheckBox[i - 1])
												if rv[i] then
													local RepeatPrmFound

													for I = 1, Ct, 1 do
														if FX[FxGUID][I] then
															if FX[FxGUID][I].Num == i - 1 then
																RepeatPrmFound = I
															end
														end
													end
									...
									if FX.LayEdit == FxGUID and Draw.DrawMode[FxGUID] ~= true then
										local L, T = r.ImGui_GetItemRectMin(ctx)
										local R, _ = r.ImGui_GetItemRectMax(ctx)
										B = T + 20
										...
										if LBtnDrag and LE.ChangingTitleSize then
											...
											if FX.TitleWidth[FxGUID] == nil then
												FX.TitleWidth[FxGUID] = DefaultWidth - 30
											end
											if Mods == 0 then
												if MouseDiff > LE.GridSize then
													FX.TitleWidth[FxGUID] = FX.TitleWidth[FxGUID] + LE.GridSize
													LE.MouseX_before = r.ImGui_GetMousePos(ctx)
													LE.BeenEdited = true
											...
											if Mods == Shift then
												FX.TitleWidth[FxGUID] = FX.TitleWidth[FxGUID] + DeltaX
												LE.BeenEdited = true
											end
										end
										if IsLBtnHeld == false then
											LE.ChangingTitleSize = nil
										end

										r.ImGui_EndDisabled(ctx)
									end

									if DebugMode and r.ImGui_IsItemHovered(ctx) then
										tooltip(FX_Idx)
									end
									if DebugMode and r.ImGui_IsKeyDown(ctx, 84) then
										tooltip(TrkID)
									end
									...
									if r.ImGui_BeginPopup(ctx, "Fx Module Menu") then
										...
										if FX.Def_Type[FxGUID] ~= "Knob" then
											...
											if ModifierHeld == Shift then
												...
												SldrW_DrgSpd = LE.GridSize
											end
											r.ImGui_SetNextItemWidth(ctx, -FLT_MIN)
											Edited, FX.Def_Sldr_W[FxGUID] = r.ImGui_DragInt(
												ctx,
												"##" .. FxGUID .. "Default Width",
												FX.Def_Sldr_W[FxGUID] or 160,
												LE.GridSize,
												50,
												300,
												"%.0f"
											...
											if Edited then
												r.SetProjExtState(
													0,
													"FX Devices",
													"Default Slider Width for FX:" .. FxGUID,
										...
										then
											if r.ImGui_Selectable(ctx, "Slider") then
												...
												r.SetProjExtState(
													0,
													"FX Devices",
													"Default Param type for FX:" .. FxGUID,
													FX.Def_Type[FxGUID]
												)
									...
									if OpenMorphSettings then
										Open, Oms = r.ImGui_Begin(
											ctx,
											"Preset Morph Settings ",
											Oms,
										...
										if Oms then
											if FxGUID == OpenMorphSettings then
												...
												if Tooltip_Timer then
													if
														r.ImGui_BeginPopupModal(
															ctx,
															"## Successfully saved preset morph",
															nil,
															r.ImGui_WindowFlags_NoTitleBar()
																| r.ImGui_WindowFlags_NoResize()
														)
												...
												if not FX[FxGUID].PrmList[1].Name then
													...
													RestoreBlacklistSettings(
														FxGUID,
														FX_Idx,
														LT_Track,
														r.TrackFX_GetNumParams(LT_Track, FX_Idx),
														FX_Name
													)
												else
													r.ImGui_BeginTable(
													...
													if --[[Last Touch]]
														LT_ParamNum and LT_FXGUID == FxGUID
													then
														...
														P.Drag_A, FX[FxGUID].MorphA[i] = r.ImGui_DragDouble(
															ctx,
															"## MorphVal_A" .. i,
															FX[FxGUID].MorphA[i],
															0.01,
														...
														P.Drag_B, FX[FxGUID].MorphB[i] = r.ImGui_DragDouble(
															ctx,
															"## MorphVal_B" .. i,
															FX[FxGUID].MorphB[i],
															0.01,
													...
													local _, FXsBL = r.GetSetMediaTrackInfo_String(
														LT_Track,
													...
													for i, v in ipairs(FX[FxGUID].PrmList) do
														...
														if
															r.ImGui_TextFilter_PassFilter(Filter, P.Name) --[[ and (i~=LT_ParamNum and LT_FXGUID==FxGUID) ]]
														then
															i = i - 1
															...
															rv, prm[i].BL =
																r.ImGui_Checkbox(ctx, "## BlackList" .. i, prm[i].BL)
															...
															if not P.FormatV_B and FX[FxGUID].MorphB[1] then
																P.FormatV_B =
																	GetFormatPrmV(FX[FxGUID].MorphB[i] or 0, OrigV, i)
															end
															...
															if P.Drag_B then
																P.FormatV_B =
																	GetFormatPrmV(FX[FxGUID].MorphB[i], OrigV, i)
															end
													...
													elseif TheresBL[1] and Save_Glob then
														file, file_path = CallFile(
															"w",
															FX.Win_Name_S[FX_Idx] .. ".ini",
															"Preset Morphing"
														)
														if file then
															for i, V in ipairs(TheresBL) do
									...
									function SyncWetValues()
										...
										if
											Wet.Val[FX_Idx] == nil
											or TrkID ~= TrkID_End
											or FXCountEndLoop ~= Sel_Track_FX_Count
										then -- if it's nil
											SyncWetValues = true
										end
										...
										elseif LT_ParamNum == FX[FxGUID].DeltaP then
											FX[FxGUID].DeltaP_V =
												r.TrackFX_GetParamNormalized(LT_Track, FX_Idx, FX[FxGUID].DeltaP)
										end
									end
									...
									if FindStringInTable(SpecialLayoutFXs, FX_Name) == false then
										...
										if FX[FXGUID[FX_Idx]].Collapse ~= true then
											Wet.ActiveAny, Wet.Active, Wet.Val[FX_Idx] =
												Add_WetDryKnob(ctx, "a", "", Wet.Val[FX_Idx] or 0, 0, 1, FX_Idx)
										end
									...
									elseif string.find(FX_Name, "Pro Q 3") ~= nil then
										if BandColor == nil then
											BandColor = 0x69B45D55
										end
										...
										if ProQ3.LT_EQBand[FXGUID[FX_Idx]] ~= nil then
											Freq_LTBandNorm = r.TrackFX_GetParamNormalized(
												LT_Track,
												FX_Idx,
											...
											local Gain = r.TrackFX_GetParamNormalized(
												LT_Track,
												FX_Idx,
										...
										r.ImGui_SetNextItemWidth(ctx, 60)
										if
											ProQ3["Freq_LTBand - " .. FXGUID[FX_Idx]] ~= nil
											and ProQ3["Freq_LTBand - " .. FXGUID[FX_Idx]] < 1000
										then
										...
										if ProQ3["Freq_LTBand - " .. FXGUID[FX_Idx]] ~= nil then
											...
											FreqDragging, FreqValueDrag[FX_Idx] = r.ImGui_DragDouble(
												ctx,
												"##FreqDrag",
												FreqValueDrag[FX_Idx],
												DragSpeed,
												-1,
												1,
												FreqLbl
										...
										if ProQ3["Gain_LTBand - " .. FXGUID[FX_Idx]] ~= nil then
											_, ProQ3.GainDrag[FX_Idx] = r.ImGui_DragDouble(
												ctx,
												"##GainDrag",
												ProQ3.GainDrag[FX_Idx] or 0,
												0.01,
												0,
												1,
										...
										if ProQ3["scaleLabel" .. " ID" .. FXGUID[FX_Idx]] ~= nil then
											DispRangeBtnClicked = r.ImGui_Button(
												ctx,
												"±"
													.. ProQ3["scaleLabel" .. " ID" .. FXGUID[FX_Idx]]
													.. "dB##"
													.. FX_Idx,
												50,
												20
										...
										if
											focusedFXState == 1
											and FX_Index_FocusFX == FX_Idx
											and LT_ParamNum == 331
										then
											_, ProQ3.DspRange[FX_Idx] =
												r.TrackFX_GetFormattedParamValue(LT_Track, FX_Idx, 331)
											...
											ProQ3["scale" .. " ID" .. FXGUID[FX_Idx]] =
												syncProQ_DispRange(ProQ3.DspRange[FX_Idx])
										end
									...
									elseif string.find(FX_Name, "Pro C 2") ~= nil then
										...
										if not FX[FXGUID[FX_Idx]].Collapse then
											if not OverSampleValue then
												_, OverSampleValue =
													r.TrackFX_GetFormattedParamValue(LT_Track, FX_Idx, 40)
											end
									...
									------------------------------------------
									if
										not FX[FXGUID[FX_Idx]].Collapse
										and FindStringInTable(BlackListFXs, FX_Name) ~= true
										and FindStringInTable(SpecialLayoutFXs, FX_Name) == false
									then
										local WinP_X
										local WinP_Y
										...
										if FX.Round[FxGUID] then
											r.ImGui_PushStyleVar(
												ctx,
												r.ImGui_StyleVar_FrameRounding(),
												FX.Round[FxGUID]
											)
										end
										...
										for Fx_P, v in ipairs(FX[FxGUID]) do --parameter faders
											...
											local F_Tp = FX.Prm.ToTrkPrm[FXGUID[FX_Idx] .. Fx_P]
											local ID = FxGUID .. Fx_P
											Rounding = 0.5
											...
											if not FP.PosX and not FP.PosY then
												if
													FP.Type == "Slider"
													or (not FP.Type and not FX.Def_Type[FxGUID])
													or FX.Def_Type[FxGUID] == "Slider"
													or FP.Type == "Drag"
													or (FX.Def_Type[FxGUID] == "Drag" and FP.Type == nil)
												then
													if Fx_P < 7 then
														r.ImGui_SetCursorPos(ctx, 0, 30 * Fx_P)
													elseif Fx_P > 6 and Fx_P <= 12 then
														r.ImGui_SetCursorPos(
															ctx,
															10
																+ (
																	FX[FxGUID][Fx_P - 6].Sldr_W
																	or FX.Def_Sldr_W[FxGUID]
																	or 160
																),
															30 * (Fx_P - 6)
														)
												...
												elseif
													FP.Type == "V-Slider"
													or (FX.Def_Type[FxGUID] == "V-Slider" and FP.Type == nil)
												then
													r.ImGui_SetCursorPos(ctx, 17 * (Fx_P - 1), 30)
												elseif
													FP.Type == "Knob" or (
														FX.Def_Type[FxGUID] == "Knob" and FP.Type == nil
													)
												then
											...
											if FP.PosX then
												r.ImGui_SetCursorPosX(ctx, FP.PosX)
											end
											if FP.PosY then
												r.ImGui_SetCursorPosY(ctx, FP.PosY)
											end
											...
											if FP.BgClr then
												...
												local RGB, _, R, G, B =
													r.ImGui_ColorConvertHSVtoRGB(H, S, HvrV or V + 0.1)
												local HvrClr = r.ImGui_ColorConvertDouble4ToU32(R, G, B, A)
												local RGB, _, R, G, B =
													r.ImGui_ColorConvertHSVtoRGB(H, S, ActV or V + 0.2)
												local ActClr = r.ImGui_ColorConvertDouble4ToU32(R, G, B, A)
												FP.BgClrHvr = HvrClr
												FP.BgClrAct = ActClr
											end
											...
											local function CheckIfCreate(
												...
												if FP[ConditionPrm] then
													...
													if FX[FxGUID][PID].ManualValues then
														...
														local _, V = r.TrackFX_GetFormattedParamValue(
															LT_Track,
															FX_Idx,
															FP[ConditionPrm]
														)
											...
											if CreateParam or not FP.ConditionPrm then
												...
												if Prm then
													...
													if
														Prm.Type == "Slider"
														or (not Prm.Type and not FX.Def_Type[FxGUID])
														or FX.Def_Type[FxGUID] == "Slider"
													then
														AddSlider(
															ctx,
															...
															Prm.Num,
															Style,
															Prm.Sldr_W or FX.Def_Sldr_W[FxGUID],
															0,
															Disable,
															Vertical,
															GrabSize,
													...
													elseif
														FP.Type == "Knob"
														or (FX.Def_Type[FxGUID] == "Knob" and Prm.Type == nil)
													then
														AddKnob(
															ctx,
															"##" .. Prm.Name,
															...
															Prm.Num,
															Prm.Style,
															Prm.Sldr_W or Df.KnobRadius,
															0,
															Disabled,
															Prm.FontSize,
															Prm.Lbl_Pos or "Bottom",
															Prm.V_Pos
														)
														MakeItemEditable(FxGUID, Fx_P, Prm.Sldr_W, "Knob", curX, CurY)
													elseif
														Prm.Type == "V-Slider" or (FX.Def_Type[FxGUID] == "V-Slider")
													then
														AddSlider(
															ctx,
															"##" .. Prm.Name,
													...
													elseif Prm.Type == "Switch" then
														AddSwitch(
															...
															Prm.Num,
															Prm.BgClr,
															Prm.CustomLbl or "Use Prm Name as Lbl",
															Fx_P,
															F_Tp,
															Prm.FontSize,
															FxGUID
													...
													elseif Prm.Type == "Drag" or (FX.Def_Type[FxGUID] == "Drag") then
														AddDrag(
															...
															Prm.Num,
															Prm.Style,
															Prm.Sldr_W or FX.Def_Sldr_W[FxGUID] or Df.Sldr_W,
															-1,
															Disable,
															Lbl_Clickable,
															Prm.Lbl_Pos,
													...
													elseif Prm.Type == "Selection" then
														AddCombo(
															...
															Prm.Name .. FxGUID,
															Prm.Num,
															FP.ManualValuesFormat or "Get Options",
															Prm.Sldr_W,
															Prm.Style,
															FxGUID,
															Fx_P,
													...
													if r.ImGui_IsItemClicked(ctx) and LBtnDC then
														...
														if file then
															...
															local P_Num = Prm.Num
															local _, P_Nm =
																r.TrackFX_GetParamName(LT_Track, FX_Idx, P_Num)
															local Df = RecallGlobInfo(
																Ct,
																P_Num .. ". " .. P_Nm .. " = ",
																"Num"
															)
										...
										if FX.LayEdit then
											if
												LE.DragY > LE.GridSize
												or LE.DragX > LE.GridSize
												or LE.DragY < -LE.GridSize
												or LE.DragX < -LE.GridSize
											then
												r.ImGui_ResetMouseDragDelta(ctx)
											end
										...
										if r.ImGui_IsMouseHoveringRect(ctx, Win_L, Win_T, Win_R, Win_B) then
											if
												ClickOnAnyItem == nil
												and LBtnRel
												and AdjustPrmWidth ~= true
												and ModifierHeld == 0
												and not SaveEditingPopupModal
											then
												LE.Sel_Items = {}
												ClickOnAnyItem = nil
												SaveEditingPopupModal = nil
									...
									if string.find(FX_Name, "Pro C 2") and not FX[FxGUID].Collapse then
										if
											...
											and FX.Win_Name[FX_Idx]:find("Pro%-C 2")
										then
										...
										if FX[FxGUID][1].Num and FX[FxGUID][8] then
											...
											for i = 1, 180, 1 do -- do once for each pixel
												...
												r.ImGui_DrawList_AddLine(
													Drawlist,
													SpX + i,
													math.min(math.max(B - Max, SpY), B),
													SpX + i + 1,
													math.min(math.max(B - MaxNext, SpY), B),
													0xFF8181cc,
													2
												)
												...
												if Wheel_V ~= 0 then
													...
													if FX[FxGUID].MsecRange then
														if FX[FxGUID].MsecRange > 999 then
															FX[FxGUID].MsecRange = round(
																(FX[FxGUID].MsecRange / 1000),
																2
															) .. "s"
														else
											...
											local MtrPreL = r.gmem_read(1002)
											if not MtrPreL then
												MtrPreL = 0
											end
											local MtrPreR = r.gmem_read(1003)
											if not MtrPreR then
												MtrPreR = 0
											end
											local MtrPoL = r.gmem_read(1001)
											if not MtrPoL then
												MtrPoL = 0
											end
											local MtrPoR = r.gmem_read(1000)
											if not MtrPoR then
												MtrPoR = 0
											end
											local MtrB = SpY + 190
											...
											for i = 0, HowManySeg, 1 do --do once for every pixel so you can get different color
												local SegL = i * ((MtrB - MtrT) / HowManySeg)
												local Clr

												local _, _, RR, GG, BB =
													r.ImGui_ColorConvertHSVtoRGB(0.4 - (0.3 / HowManySeg) * i, 0.6, 0.5)
											...
											if MtrPreL > 0 then
												PreLPeak = true
												PlayStateWhenPeak = r.GetPlayState()
											end
											if MtrPreR > 0 then
												PreRPeak = true
												PlayStateWhenPeak = r.GetPlayState()
											...
											for Fx_p = 1, 4, 1 do
												...
												if FX[FxGUID][Fx_P].V == nil then
													FX[FxGUID][Fx_P].V =
														r.TrackFX_GetParamNormalized(LT_Track, FX_Idx, P_Num)
												end
											...
											if
												FX.Win_Name[FX_Idx - 1]:find("JS: FXD Split to 4 channels")
												and not tablefind(Trk[TrkID].PreFX, FxGUID)
												and not tablefind(Trk[TrkID].PostFX, FxGUID)
											then
												table.insert(AddFX.Pos, FX_Idx)
											...
											if
												not NextFX:find("JS: FXD Gain Reduction Scope")
												and not tablefind(Trk[TrkID].PreFX, FxGUID)
												and not tablefind(Trk[TrkID].PostFX, FxGUID)
											then
												table.insert(AddFX.Pos, FX_Idx + 1)
												...
												function WriteGmemToGainReductionScope(FxGUID) end

												if not GainReductionWait then
													GainReductionWait = 0
												end
									...
									if string.find(FX_Name, "Pro Q 3") ~= nil and FX[FxGUID].Collapse ~= true then -- ==  Pro Q Graph
										...
										then
											if ProQ3["scale" .. " ID" .. FXGUID[FX_Idx]] == nil then
												ProQ3["scale" .. " ID" .. FXGUID[FX_Idx]] = 2.5
											end
											...
											for i = 2, 249, 1 do
												...
												if lx ~= tx and i ~= 2 then
													r.ImGui_DrawList_AddQuadFilled(
														Foreground,
														lx,
														B,
														lx,
											...
											for Band = 1, 24, 1 do
												...
												if FXGUID_ProQ == nil then
													FXGUID_ProQ = 0
												end
												ProQ3.Band_UseState[Band] =
													r.TrackFX_GetParamNormalized(LT_Track, FX_Idx, paramOfUsed)
												Band_Enabled[Band .. FXGUID_ProQ] =
													r.TrackFX_GetParamNormalized(LT_Track, FX_Idx, paramOfEnabled)
												local x = ProQ_Xpos_L
												local y = Y_Mid
												if ProQ3.Band_UseState[Band] == 1 then
													Freq[Band] =
														r.TrackFX_GetParamNormalized(LT_Track, FX_Idx, paramOfUsed + 2)
													Slope[Band] = r.TrackFX_GetParamNormalized(
														LT_Track,
														FX_Idx,
														9 + 13 * (Band - 1)
													)
													Gain[Band] =
														r.TrackFX_GetParamNormalized(LT_Track, FX_Idx, paramOfUsed + 3)
													_, ProQ3["Shape of Band" .. Band .. "ID" .. FXGUID_ProQ] =
														r.TrackFX_GetFormattedParamValue(
															LT_Track,
															FX_Idx,
															paramOfUsed + 8
														)
													NodeFreq["B-" .. Band .. "GUID-" .. FXGUID_ProQ] =
														x_to_freq(Freq[Band] * ProQ3.Width)
													if
														ProQ3["Shape of Band" .. Band .. "ID" .. FXGUID_ProQ]
															== "Low Shelf"
														or ProQ3["Shape of Band" .. Band .. "ID" .. FXGUID_ProQ]
															== "High Shelf"
													then
													...
													if ProQ3.Format == "AU" then
														Q[Band] = r.TrackFX_GetParamNormalized(
															LT_Track,
															FX_Idx,
															paramOfUsed + 7
														)
														...
														if
															...
															or ProQ3["Shape of Band" .. Band .. "ID" .. FXGUID_ProQ]
																== "High Cut"
														then
															_, Q[Band] = r.TrackFX_GetFormattedParamValue(
																LT_Track,
																FX_Idx,
																paramOfUsed + 7
															)
													...
													Q_Node[Band] =
														r.TrackFX_GetParamNormalized(LT_Track, FX_Idx, paramOfUsed + 7)
													...
													if Band_Enabled[Band .. FXGUID_ProQ] == 1 then
														for i = 1, ProQ3.Width, 2 do -- do 340 times
															...
															pts[i .. "B-" .. Band .. FXGUID_ProQ] = (
																(pts[i .. "B-" .. Band .. FXGUID_ProQ] + mag) / 2
															) * ProQ3["scale" .. " ID" .. FXGUID[FX_Idx]]
															FillClr_LT_Band(i, y)
															if ProQ3.LT_EQBand[FXGUID_ProQ] == Band then
																X2 = x + 2
																BandColor =
																	determineBandColor(ProQ3.LT_EQBand[FXGUID_ProQ])
																if i ~= 1 then
																	r.ImGui_DrawList_AddLine(
																		Foreground,
																		x,
												...
												if
													...
													and ProQ3["Shape of Band" .. Band .. "ID" .. FXGUID_ProQ] == "Bell"
												then
													...
													if Band_Enabled[Band .. FXGUID_ProQ] == 1 then
														for i = 1, ProQ3.Width, 2 do
															...
															pts[i .. "B-" .. Band .. FXGUID_ProQ] = (
																Gain_Math * Euler ^ -(Q_Math * (i - Freq_Math)) ^ 2
															) * ProQ3["scale" .. " ID" .. FXGUID[FX_Idx]]
												...
												elseif
													ProQ3.Band_UseState[Band] == 1.0
													and ProQ3["Shape of Band" .. Band .. "ID" .. FXGUID_ProQ]
														== "High Cut"
												then
												...
												elseif
													ProQ3.Band_UseState[Band] == 1.0
													and ProQ3["Shape of Band" .. Band .. "ID" .. FXGUID_ProQ] == "Low Cut"
												then
													if Slope[Band] < 0.2 then
														MIN_Q = 0.1
														MAX_Q = 200
												...
												elseif
													...
													and ProQ3["Shape of Band" .. Band .. "ID" .. FXGUID_ProQ]
														== "Low Shelf"
												then --@todo Pro Q -- No support for different slopes
													MIN_Q = 0.1
													MAX_Q = 100
												...
												elseif
													ProQ3.Band_UseState[Band] == 1.0
													and ProQ3["Shape of Band" .. Band .. "ID" .. FXGUID_ProQ] == "Notch"
												then
													MIN_Q = 0.005
													MAX_Q = 400
													Q[Band] = per_to_q(Q[Band] * 100, 100)
											...
											for i = 1, ProQ3.Width, 2 do
												...
												for Band = 1, 24, 1 do --Add up the sum of all eq
													if ProQ3.Band_UseState[Band] == 1 then
														if
															pts[i .. "B-" .. Band .. FXGUID_ProQ] ~= nil
															and Band_Enabled[Band .. FXGUID_ProQ] == 1
														then
											...
											if
												ProQ3["scaleLabel" .. " ID" .. FXGUID[FX_Idx]] == 30
												or ProQ3["scaleLabel" .. " ID" .. FXGUID[FX_Idx]] == 3
											then
											...
											for Band = 1, 24, 1 do
												if ProQ3.Band_UseState[Band] == 1 then
													NodeFreq["B-" .. Band .. "GUID-" .. FXGUID_ProQ] =
														x_to_freq(Freq[Band] * ProQ3.Width)
													XposNode[Band] =
														freq_to_scx(NodeFreq["B-" .. Band .. "GUID-" .. FXGUID_ProQ])
													_, ProQ3["Shape of Band" .. Band .. "ID" .. FXGUID_ProQ] =
														r.TrackFX_GetFormattedParamValue(
															LT_Track,
															FX_Idx,
													...
													if
														ProQ3["Shape of Band" .. Band .. "ID" .. FXGUID_ProQ] == "Bell"
													then
													...
													then
														NodeY_Pos[Band] = Y_Mid
													elseif
														ProQ3["Shape of Band" .. Band .. "ID" .. FXGUID_ProQ] == "Notch"
													then
														NodeY_Pos[Band] = Y_Mid
													elseif
														ProQ3["Shape of Band" .. Band .. "ID" .. FXGUID_ProQ]
													...
													if Band_Enabled[Band .. FXGUID_ProQ] == 1 then
														...
														r.ImGui_DrawList_AddCircleFilled(
															Foreground,
															ProQ_Xpos_L + (XposNode[Band] or 0),
															NodeY_Pos[Band] or 0,
															6,
															Clr_HalfAlpha
														)
													...
													if
														MousePosX > ProQ_Xpos_L + XposNode[Band] - NodeHoverArea
														and MousePosX < ProQ_Xpos_L + XposNode[Band] + NodeHoverArea
														and MousePosY > NodeY_Pos[Band] - NodeHoverArea
														and MousePosY < NodeY_Pos[Band] + NodeHoverArea
													then
													...
													then
														...
														if IsLBtnHeld then
															...
															if IsLBtnClicked then
																ProQ3["NodeDrag" .. Band .. " ID-" .. FXGUID[FX_Idx]] =
																	true
															end
														end

														local QQ = r.TrackFX_GetParamNormalized(
															LT_Track,
															FX_Idx,
															((Band - 1) * 13) + 7
														)
														...
														if IsLBtnClicked and ModifierHeld == Alt then -- delete node
															...
															ProQ3["NodeHvr" .. Band .. "FXID-" .. FXGUID[FX_Idx]] =
																false
															HvringNode = nil
														end

														if LBtnClickCount == 2 then
															local OnOff = r.TrackFX_GetParamNormalized(
																LT_Track,
																FX_Idx,
																((Band - 1) * 13) + 1
															)
													...
													if ProQ3["AdjustingQ" .. FXGUID[FX_Idx]] then
														...
														if
															MousePosX_AdjustingQ_CheckXpos
																< MousePosX_AdjustingQ + 7
															and MousePosX_AdjustingQ_CheckXpos
																> MousePosX_AdjustingQ - 7
														then
															local QQ = r.TrackFX_GetParamNormalized(
																LT_Track,
															...
															Q_Output =
																SetMinMax(QQ - ((Wheel_V / 50) / WheelQFineAdj), 0, 1)
													...
													if
														ProQ3["NodeDrag" .. Band .. " ID-" .. FXGUID[FX_Idx]] == true
													then
														MouseDeltaX, MouseDeltaY = r.ImGui_GetMouseDelta(ctx)
														local Freq = r.TrackFX_GetParamNormalized(
															LT_Track,
															FX_Idx,
															((Band - 1) * 13) + 2
														)
														...
														local Q = r.TrackFX_GetParamNormalized(
															LT_Track,
															FX_Idx,
															((Band - 1) * 13) + 7
														)
											...
											if ProQ3["HvrGUI" .. FXGUID[FX_Idx]] and LBtnClickCount == 2 then
												if
													HvringNode == nil
													or ProQ3["NodeHvr" .. HvringNode .. "FXID-" .. FXGUID[FX_Idx]] ~= true
												then
													...
													local GainToAddNode = (
														(((Y_Mid - MouseY_AddNode) - 100) / 100 + 1)
															/ ProQ3["scale" .. " ID" .. FXGUID[FX_Idx]]
														+ 1
													) / 2
													reaper.TrackFX_SetParamNormalized(
											...
											if FX[FxGUID].AddEQSpectrumWait > FX_Add_Del_WaitTime then
												...
												if
													r.ImGui_IsPopupOpen(
														ctx,
														"Delete FX Layer ",
														r.ImGui_PopupFlags_AnyPopupId()
															+ r.ImGui_PopupFlags_AnyPopupLevel()
													)
												then
													AnyPopupOpen = true
												end
												...
												if
													not tablefind(Trk[TrkID].PostFX, FxGUID)
													and not tablefind(Trk[TrkID].PreFX, FxGUID)
													and not AnyPopupOpen
												then
													r.gmem_attach("gmemReEQ_Spectrum")
													r.gmem_write(1, PM.DIY_TrkID[TrkID])
													FX[FxGUID].ProQ_ID = FX[FxGUID].ProQ_ID
														or math.random(1000000, 9999999)
													r.GetSetMediaTrackInfo_String(
														LT_Track,
														"P_EXT: ProQ_ID " .. FxGUID,
														FX[FxGUID].ProQ_ID,
						...
						if BlinkFX == FX_Idx then
							BlinkFX = BlinkItem(0.2, 2, BlinkFX)
						end
					end --of Create fx window function

					if--[[Normal Window]]
						(not string.find(FX_Name, "FXD %(Mix%)RackMixer"))
						and FX.InLyr[FXGUID[FX_Idx]] == nil
						and FX_Idx ~= RepeatTimeForWindows
						and FindStringInTable(BlackListFXs, FX_Name) ~= true
					then
						...
						if FX.LayEdit == FXGUID[FX_Idx] then
							if not ctx then
								ctx = r.ImGui_CreateContext("Layout Editor")
							end
							--r.ImGui_BeginTooltip( ctx)

							--attachfonts(ctx)

							rv, LayEdProp_Open = r.ImGui_Begin(
								ctx,
								"LayoutEdit Propertiess",
								true,
								r.ImGui_WindowFlags_MenuBar()
									+ r.ImGui_WindowFlags_NoCollapse()
									+ r.ImGui_WindowFlags_NoTitleBar()
							...
							if rv and LayEdProp_Open then
								...
								if not LE.Sel_Items[1] then
									if Draw.DrawMode[FxGUID] ~= true then
										...
										if r.ImGui_Button(ctx, "Exit Draw Mode ") then
											Draw.DrawMode[FxGUID] = false
										end
										...
										if Draw.SelItm then
											typelbl = D.Type[Draw.SelItm]
										end
										if Draw.Type == nil then
											Draw.Type = "line"
										end
										if
											r.ImGui_BeginCombo(
												ctx,
												"##",
												typelbl or Draw.Type or "line",
												r.ImGui_ComboFlags_NoArrowButton()
											)
										then
											if r.ImGui_Selectable(ctx, "line") then
												if Draw.SelItm then
													D.Type[Draw.SelItm] = "line"
												end
												Draw.Type = "line"
											end
										...
										if Draw.SelItm and D.clr[Draw.SelItm] then
											clrpick, D.clr[Draw.SelItm] = r.ImGui_ColorEdit4(
												ctx,
												"##",
										...
										r.ImGui_SameLine(ctx)
										r.ImGui_SetNextItemWidth(ctx, 40)
										EditER, Draw.Df_EdgeRound[FxGUID] = r.ImGui_DragDouble(
											ctx,
											"##" .. FxGUID,
										...
										if Draw.SelItm then
											...
											if
												D.Type[It] ~= "V-line"
												and D.Type[It] ~= "circle"
												and D.Type[It] ~= "circle fill"
											then
												r.ImGui_Text(ctx, "End Pos X:")
												r.ImGui_SameLine(ctx)
												_, D.R[It] = r.ImGui_DragDouble(
													ctx,
													"##" .. Draw.SelItm .. "R",
													D.R[Draw.SelItm],
													1,
											...
											r.ImGui_SameLine(ctx)
											_, D.T[Draw.SelItm] = r.ImGui_DragDouble(
												ctx,
												"##" .. Draw.SelItm .. "T",
												D.T[Draw.SelItm],
												1,
								...
								elseif LE.Sel_Items[1] then
									local ID, TypeID
									local FrstSelItm = FX[FxGUID][LE.Sel_Items[1]]
									local FItm = LE.Sel_Items[1]
									local R_ofs = 50
									...
									elseif LE.Sel_Items[2] then
										...
										for i, v in pairs(LE.Sel_Items) do
											local lastV
											if i > 1 then
												local frst = LE.Sel_Items[1]
												local other = LE.Sel_Items[i]
												if FX[FxGUID][1].Type ~= FX[FxGUID][v].Type then
									...
									local function FreeValuePosSettings()
										if FrstSelItm.V_Pos == "Free" then
											...
											SL()
											r.ImGui_SetNextItemWidth(ctx, 50)
											local EditPosX, PosX = r.ImGui_DragDouble(
												ctx,
											...
											if EditPosX then
												for i, v in pairs(LE.Sel_Items) do
													FrstSelItm.V_Pos_X = PosX
												end
											end
											r.ImGui_Text(ctx, "Y:")
											SL()
											r.ImGui_SetNextItemWidth(ctx, 50)
											local EditPosY, PosY = r.ImGui_DragDouble(
												ctx,
									...
									if not FrstSelItm.Type then
										FrstSelItm.Type = FX.Def_Type[FxGUID]
									end
									r.ImGui_Text(ctx, "Type : ")
									r.ImGui_SameLine(ctx)
									r.ImGui_PushStyleColor(ctx, r.ImGui_Col_FrameBg(), 0x444444aa)
									r.ImGui_SetNextItemWidth(ctx, -R_ofs)
									if
										r.ImGui_BeginCombo(ctx, "##", PrmTypeLbl, r.ImGui_ComboFlags_NoArrowButton())
									then
									...
									if LE.Sel_Items[1] and not LE.Sel_Items[2] then
										...
										r.ImGui_SameLine(ctx)
										r.ImGui_SetNextItemWidth(ctx, -R_ofs)
										local LblEdited, buf = r.ImGui_InputText(
											ctx,
											" ##Edit Title" .. FxGUID .. LE.Sel_Items[1],
											FrstSelItm.CustomLbl or buf
										)
										if r.ImGui_IsItemActivated(ctx) then
											EditingPrmLbl = LE.Sel_Items[1]
										end
										if r.ImGui_IsItemDeactivatedAfterEdit(ctx) then
											FrstSelItm.CustomLbl = buf
										end
									end

									--Label Pos
									r.ImGui_Text(ctx, "Label Pos: ")
									r.ImGui_SameLine(ctx)
									r.ImGui_SetNextItemWidth(ctx, 100)
									if
										r.ImGui_BeginCombo(
											ctx,
											"## Lbl Pos" .. LE.Sel_Items[1],
											FrstSelItm.Lbl_Pos or "Default",
											r.ImGui_ComboFlags_NoArrowButton()
										)
									then
									...
									FreeLblPosSettings()
									-- Label Color
									DragLbl_Clr_Edited, Lbl_V_Clr = r.ImGui_ColorEdit4(
										ctx,
										"##Lbl Clr" .. LE.Sel_Items[1],
									...
									if DragLbl_Clr_Edited then
										for i, v in pairs(LE.Sel_Items) do
											FX[FxGUID][v].Lbl_Clr = Lbl_V_Clr
										end
									end

									r.ImGui_Text(ctx, "Value Pos: ")
									r.ImGui_SameLine(ctx)
									r.ImGui_SetNextItemWidth(ctx, 100)
									if
										r.ImGui_BeginCombo(
											ctx,
											"## V Pos" .. LE.Sel_Items[1],
											FrstSelItm.V_Pos or "Default",
											r.ImGui_ComboFlags_NoArrowButton()
										)
									then
									...
									DragV_Clr_edited, Drag_V_Clr = r.ImGui_ColorEdit4(
										ctx,
									...
									if FrstSelItm.Type == "Drag" then
										...
										if
											r.ImGui_BeginCombo(
												ctx,
												"## Drag Dir" .. LE.Sel_Items[1],
												FrstSelItm.DragDir or "",
												r.ImGui_ComboFlags_NoArrowButton()
											)
										then
											if r.ImGui_Selectable(ctx, "Right") then
												for i, v in pairs(LE.Sel_Items) do
													FX[FxGUID][v].DragDir = "Right"
												end
											elseif r.ImGui_Selectable(ctx, "Left-Right") then
												for i, v in pairs(LE.Sel_Items) do
													FX[FxGUID][v].DragDir = "Left-Right"
												end
											elseif r.ImGui_Selectable(ctx, "Left") then
												for i, v in pairs(LE.Sel_Items) do
													FX[FxGUID][v].DragDir = "Left"
												end
											end
											r.ImGui_EndCombo(ctx)
										end
									...
									if FrstSelItm.Type == "Switch" then
										...
										r.ImGui_SameLine(ctx)
										EdM, Mt =
											r.ImGui_Checkbox(ctx, "Momentary##" .. FxGUID .. LE.Sel_Items[1], Momentary)
										if EdT then
											for i, v in pairs(LE.Sel_Items) do
										...
										elseif EdM then
											for i, v in pairs(LE.Sel_Items) do
												FX[FxGUID][v].SwitchType = "Momentary"
											end
										end
									end
									...
									if FrstSelItm.SwitchType == "Momentary" and FrstSelItm.Type == "Switch" then
										...
										local Drag, Bv = r.ImGui_DragDouble(
											ctx,
											"##EditBaseV" .. FxGUID .. (LE.Sel_Items[1] or ""),
											FX[FxGUID][LE.Sel_Items[1]].SwitchBaseV or 0,
											0.05,
											0,
											1,
											"%.2f"
										...
										r.ImGui_SameLine(ctx)
										r.ImGui_SetNextItemWidth(ctx, 80)
										local Drag, Tv = r.ImGui_DragDouble(
											ctx,
											"##EditTargV" .. FxGUID .. (LE.Sel_Items[1] or ""),
											FX[FxGUID][LE.Sel_Items[1]].SwitchTargV or 1,
											0.05,
											0,
											1,
											"%.2f"
										...
										if Drag then
											for i, v in pairs(LE.Sel_Items) do
												FX[FxGUID][LE.Sel_Items[1]].SwitchTargV = Tv
											end
										end
									end
									...
									r.ImGui_Text(ctx, "Label Font Size: ")
									r.ImGui_SameLine(ctx)
									r.ImGui_SetNextItemWidth(ctx, 50)
									Drag, ft = r.ImGui_DragDouble(
										ctx,
										"##EditFontSize" .. FxGUID .. (LE.Sel_Items[1] or ""),
										FX[FxGUID][LE.Sel_Items[1]].FontSize or Knob_DefaultFontSize,
										0.25,
										6,
										16,
										"%.2f"
									...
									SL()
									r.ImGui_Text(ctx, "Value Font Size: ")
									r.ImGui_SameLine(ctx)
									r.ImGui_SetNextItemWidth(ctx, 50)
									local Drag, ft = r.ImGui_DragDouble(
										ctx,
										"##EditV_FontSize" .. FxGUID .. (LE.Sel_Items[1] or ""),
										FX[FxGUID][LE.Sel_Items[1]].V_FontSize or Knob_DefaultFontSize,
										0.25,
										6,
										16,
										"%.2f"
									...
									if FrstSelItm.Type == "Knob" then
										...
										MinW = 7.5
									elseif
										FrstSelItm.Type == "Slider"
										or FrstSelItm.Type == "Drag"
										or not FrstSelItm.Type
									then
										DefaultW = Df.Sldr_W
										MaxW = 300
										MinW = 40
									...
									local _, W = r.ImGui_DragDouble(
										ctx,
										"##EditWidth" .. FxGUID .. (LE.Sel_Items[1] or ""),
										FX[FxGUID][LE.Sel_Items[1] or ""].Sldr_W or DefaultW,
										LE.GridSize / 4,
										MinW,
										MaxW,
										"%.1f"
									...
									if
										FrstSelItm.Type == "Knob"
										or FrstSelItm.Type == "Drag"
										or FrstSelItm.Type == "Slider"
									then
										r.ImGui_Text(ctx, "Value Decimal Places: ")
										r.ImGui_SameLine(ctx)
										r.ImGui_SetNextItemWidth(ctx, 80)
										if not FX[FxGUID][LE.Sel_Items[1]].V_Round then
											local _, FormatV = r.TrackFX_GetFormattedParamValue(
												LT_Track,
												FX_Idx,
										...
										local Edit, rd = r.ImGui_InputInt(
											ctx,
											"##EditValueDecimals" .. FxGUID .. (LE.Sel_Items[1] or ""),
											FrstSelItm.V_Round or rd,
											1
										)
										if Edit then
											for i, v in pairs(LE.Sel_Items) do
									...
									r.ImGui_Text(ctx, "Value to Note Length: ")
									r.ImGui_SameLine(ctx)
									r.ImGui_SetNextItemWidth(ctx, 80)
									local Edit = r.ImGui_Checkbox(
										ctx,
									...
									if FrstSelItm.Type == "Selection" then --r.ImGui_Text(ctx,'Edit Values Manually: ') ;r.ImGui_SameLine(ctx)
										...
										if r.ImGui_TreeNode(ctx, "Edit Values Manually") then
											...
											FX[FxGUID][Itm].ManualValuesFormat = FX[FxGUID][Itm].ManualValuesFormat
												or {}
											...
											for i, V in ipairs(FX[FxGUID][Itm].ManualValues) do
												...
												SL()
												r.ImGui_SetNextItemWidth(ctx, -R_ofs)
												rv, FX[FxGUID][Itm].ManualValuesFormat[i] = r.ImGui_InputText(
													ctx,
									...
									local stylename
									if FrstSelItm.Style == "Pro C" then
										stylename = "Minimalistic"
									end
									if
										r.ImGui_Button(
											ctx,
											(stylename or FrstSelItm.Style or "Choose Style")
												.. "##"
												.. (LE.Sel_Items[1] or "Style"),
											155
										)
									then
									...
									if r.ImGui_BeginPopup(ctx, "Choose style window") then
										...
										if
											FrstSelItm.Type == "Slider"
											or (not FrstSelItm.Type and FX.Def_Type[FxGUID] == "Slider")
										then -- if all selected itms are Sliders
										...
										if
											FrstSelItm.Type == "Knob"
											or (not FrstSelItm.Type and FX.Def_Type[FxGUID] == "Knob")
										then -- if all selected itms are knobs
									...
									r.ImGui_Text(ctx, "Pos-X: ")
									r.ImGui_SameLine(ctx)
									r.ImGui_SetNextItemWidth(ctx, 80)
									local EditPosX, PosX = r.ImGui_DragDouble(
										ctx,
										" ##EditPosX" .. FxGUID .. LE.Sel_Items[1],
										PosX or FrstSelItm.PosX,
										LE.GridSize,
										0,
										Win_W - 10,
										"%.0f"
									...
									if EditPosX then
										for i, v in pairs(LE.Sel_Items) do
											FX[FxGUID][v].PosX = PosX
										end
									end
									SL()
									r.ImGui_Text(ctx, "Pos-Y: ")
									r.ImGui_SameLine(ctx)
									r.ImGui_SetNextItemWidth(ctx, 80)
									local EditPosY, PosY = r.ImGui_DragDouble(
										ctx,
										" ##EditPosY" .. FxGUID .. LE.Sel_Items[1],
										PosY or FrstSelItm.PosY,
										LE.GridSize,
										20,
										210,
										"%.0f"
									...
									r.ImGui_Text(ctx, "Color: ")
									r.ImGui_SameLine(ctx)
									ClrEdited, PrmBgClr = r.ImGui_ColorEdit4(
										ctx,
										"##Clr" .. ID,
									...
									if
										...
										or FX[FxGUID][LE.Sel_Items[1]]
											== r.ImGui_GetColor(ctx, r.ImGui_Col_FrameBg())
									then
										HighlightSelectedItem(nil, 0xffffffdd, 0, L, T, R, B, h, w, 0, 0, "GetItemRect")
									end
									if ClrEdited then
										for i, v in pairs(LE.Sel_Items) do
											FX[FxGUID][v].BgClr = PrmBgClr
										end
									end
									...
									if FrstSelItm.Type ~= "Switch" and FrstSelItm.Type ~= "Selection" then
										...
										r.ImGui_SameLine(ctx)
										GrbClrEdited, GrbClr = r.ImGui_ColorEdit4(
											ctx,
											"##GrbClr" .. ID,
										...
										if
											...
											or FX[FxGUID][LE.Sel_Items[1]].GrbClr
												== r.ImGui_GetColor(ctx, r.ImGui_Col_SliderGrab())
										then
											HighlightSelectedItem(
												nil,
												0xffffffdd,
									...
									if FrstSelItm.Type == "Knob" then
										...
										SL()
										r.ImGui_SetNextItemWidth(ctx, 40)
										local TD, Thick = r.ImGui_DragDouble(
											ctx,
											"##EditValueFontSize" .. FxGUID .. (LE.Sel_Items[1] or ""),
											FX[FxGUID][LE.Sel_Items[1] or ""].Value_Thick or 2,
											0.1,
											0.5,
											8,
											"%.1f"
									...
									if FrstSelItm.Type == "Selection" then
										...
										r.ImGui_SameLine(ctx)
										local DragLbl_Clr_Edited, V_Clr = r.ImGui_ColorEdit4(
											ctx,
											"##V Clr" .. LE.Sel_Items[1],
											FX[FxGUID][LE.Sel_Items[1] or ""].V_Clr
												or r.ImGui_GetColor(ctx, r.ImGui_Col_Text()),
											r.ImGui_ColorEditFlags_NoInputs()
												| r.ImGui_ColorEditFlags_AlphaPreviewHalf()
												| r.ImGui_ColorEditFlags_AlphaBar()
										)
									...
									elseif FrstSelItm.Type == "Switch" then
										...
										local DragLbl_Clr_Edited, V_Clr = r.ImGui_ColorEdit4(
											...
											FX[FxGUID][LE.Sel_Items[1] or ""].Switch_On_Clr
												or r.ImGui_GetColor(ctx, r.ImGui_Col_Text()),
											r.ImGui_ColorEditFlags_NoInputs()
												| r.ImGui_ColorEditFlags_AlphaPreviewHalf()
												| r.ImGui_ColorEditFlags_AlphaBar()
									...
									local function Condition(
										...
										if r.ImGui_Button(ctx, BtnTitle) then
											if ModifierHeld == 0 then
												for i, v in pairs(LE.Sel_Items) do
													if not FX[FxGUID][v][ShowCondition] then
														FX[FxGUID][v][ShowCondition] = true
													else
														FX[FxGUID][v][ShowCondition] = nil
													end
										...
										if r.ImGui_IsItemHovered(ctx) then
											HintToolTip("Alt-Click to Delete All Conditions")
										end
										...
										if FrstSelItm[ShowCondition] or FX[FxGUID][P][ConditionPrm] then
											...
											local Edit, Cond = r.ImGui_InputInt(
												ctx,
											...
											Value_Selected, V_Formatted = AddCombo(
												ctx,
												LT_Track,
												FX_Idx,
												ConditionPrm .. (PrmName or "") .. "1",
												FX[FxGUID][P][ConditionPrm] or 0,
												...
												FX[FxGUID][P][ConditionPrm_V][1] or "Unassigned",
												nil,
												"No Lbl"
											)
											...
											if Value_Selected then
												for i, v in pairs(LE.Sel_Items) do
													...
													FX[FxGUID][v][ConditionPrm_V] = FX[FxGUID][v][ConditionPrm_V] or {}
													FX[FxGUID][v][ConditionPrm_V_Norm] = FX[FxGUID][v][ConditionPrm_V_Norm]
														or {}
													FX[FxGUID][v][ConditionPrm_V][1] = V_Formatted
													FX[FxGUID][v][ConditionPrm_V_Norm][1] =
														r.TrackFX_GetParamNormalized(
															LT_Track,
															FX_Idx,
															FX[FxGUID][P][ConditionPrm]
												...
												if FX[FxGUID][P][ConditionPrm_V][2] then
													for i, v in pairs(FX[FxGUID][P][ConditionPrm_V]) do
														if i > 1 then
															...
															r.ImGui_SameLine(ctx)
															local Value_Selected, V_Formatted = AddCombo(
																ctx,
																LT_Track,
																FX_Idx,
																"CondPrmV" .. (PrmName or "") .. v .. ConditionPrm,
																FX[FxGUID][P][ConditionPrm] or 0,
																FX[FxGUID][PID].ManualValuesFormat or "Get Options",
																Width,
																Style,
															...
															if Value_Selected then
																for I, v in pairs(LE.Sel_Items) do
																	...
																	FX[FxGUID][v][ConditionPrm_V_Norm][i] =
																		r.TrackFX_GetParamNormalized(
																			LT_Track,
																			FX_Idx,
																			FX[FxGUID][P][ConditionPrm]
								...
								if LE.SelectedItem == "Title" then
									...
									r.ImGui_SameLine(ctx)
									Edited, FX.Round[FxGUID] = r.ImGui_DragDouble(
										ctx,
										"##" .. FxGUID .. "Round",
										FX.Round[FxGUID],
										0.01,
									...
									r.ImGui_SameLine(ctx)
									Edited, FX.GrbRound[FxGUID] = r.ImGui_DragDouble(
										ctx,
										"##" .. FxGUID .. "GrbRound",
										FX.GrbRound[FxGUID],
										0.01,
									...
									r.ImGui_SameLine(ctx)
									_, FX.BgClr[FxGUID] = r.ImGui_ColorEdit4(
										ctx,
										"##" .. FxGUID .. "BgClr",
										FX.BgClr[FxGUID] or FX_Devices_Bg or 0x151515ff,
										r.ImGui_ColorEditFlags_NoInputs()
									...
									r.ImGui_SameLine(ctx)
									_, FX[FxGUID].TitleClr = r.ImGui_ColorEdit4(
										ctx,
										"##" .. FxGUID .. "Title Clr",
									...
									r.ImGui_SameLine(ctx)
									local _, CustomTitle = r.ImGui_InputText(
										ctx,
										"##CustomTitle" .. FxGUID,
								...
								if Draw.DrawMode[FxGUID] == true then
									if not CheckIfDrawingHasBeenMade(FX_Idx) then
										r.ImGui_BeginDisabled(ctx)
									end
									...
									if r.ImGui_Button(ctx, "Delete") then
										...
										if not FX[FxGUID][1] then
											FX[FxGUID].AllPrmHasBeenDeleted = true
										else
											FX[FxGUID].AllPrmHasBeenDeleted = nil
										end
								...
								if r.ImGui_BeginPopupModal(ctx, "Save Editing?") then
									...
									if
										r.ImGui_Button(ctx, "(c) Cancel")
										or r.ImGui_IsKeyPressed(ctx, 67)
										or r.ImGui_IsKeyPressed(ctx, 27)
									then
										r.ImGui_CloseCurrentPopup(ctx)
									end
								...
								for Pal = 1, NumOfColumns or 1, 1 do
									...
									then
										...
										for i, v in ipairs(ClrPallet) do
											clrpick, LblColor1 = r.ImGui_ColorEdit4(
												ctx,
												"##ClrPalette" .. Pal .. i .. FxGUID,
												v,
												r.ImGui_ColorEditFlags_NoInputs()
													| r.ImGui_ColorEditFlags_AlphaPreviewHalf()
													| r.ImGui_ColorEditFlags_AlphaBar()
					...
					elseif--[[FX Layer Window ]]
						string.find(FX_Name, "FXD %(Mix%)RackMixer") or string.find(FX_Name, "FXRack")
					then --!!!!  FX Layer Window
						if not FX[FxGUID].Collapse then
							FXGUID_RackMixer = r.TrackFX_GetFXGUID(LT_Track, FX_Idx)
							...
							if
								r.ImGui_BeginChildFrame(
									ctx,
									"##FX Layer at" .. FX_Idx .. "OnTrack " .. TrkID,
									FXLayeringWin_X + Pad,
									220,
									r.ImGui_WindowFlags_NoScrollbar()
								)
							then
								...
								FXLayerFrame_PosX_R, FXLayerFrame_PosY_B = r.ImGui_GetItemRectMax(ctx)
								FXLayerFrame_PosY_B = FXLayerFrame_PosY_B + 220
								...
								if
									r.ImGui_BeginDragDropSource(ctx, r.ImGui_DragDropFlags_AcceptNoDrawDefaultRect())
								then
									DragFX_ID = FX_Idx
								...
								elseif r.ImGui_IsItemClicked(ctx) and ModifierHeld == Shift then
									local Spltr, FX_Inst
									if FX[FxGUID].LyrDisable == nil then
										FX[FxGUID].LyrDisable = false
									end
									...
									if FX[FxGUID].LyrDisable then
										FX[FxGUID].LyrDisable = false
									else
										FX[FxGUID].LyrDisable = true
									end
								end
								...
								if not FX[FxGUID].RenameFXLayering then
									...
									_, FX[FxGUID].ContainerTitle = r.ImGui_InputText(
										ctx,
										"##" .. FxGUID,
										FX[FxGUID].ContainerTitle or "FX Layering",
										Flag
									)
								...
								if
									r.ImGui_Button(
										ctx,
										(Lbl or "") .. "##FX Lyr Mode" .. FxGUID,
										30,
										r.ImGui_GetTextLineHeight(ctx)
									)
								then
									FX[FxGUID].SumMode = r.TrackFX_GetParamNormalized(LT_Track, FX_Idx, 40)
								...
								elseif FX[FxGUID].ActiveLyrCount == 5 then
									LineH, Spacing, Inner_Spacing = 3, -5, 0
									BtnSizeManual = 30
									MaxChars = 18
								elseif FX[FxGUID].ActiveLyrCount == 6 then
									LineH, Spacing, Inner_Spacing = 5.5, -5, -8
									BtnSizeManual = 24
									MaxChars = 20
								elseif FX[FxGUID].ActiveLyrCount >= 7 then
									LineH, Spacing, Inner_Spacing = 3, -5, -8
									BtnSizeManual = 19
									MaxChars = 23
								end
								...
								if not AnySoloChan then
									FX[FxGUID].AnySoloChan = nil
								end
								...
								for LayerNum, LyrID in pairs(FX[FxGUID].LyrID) do
									...
									for i = 0, Sel_Track_FX_Count, 1 do
										if
											FX.InLyr[FXGUID[i]] == FXGUID_RackMixer
											and FX[FXGUID[i]].inWhichLyr == LyrID
										then
									...
									if FX[FxGUID][Fx_P] then
										...
										if Lyr.Rename[LyrID .. FxGUID] ~= true and Fx_P then
											...
											AddDrag(
												...
												P_Num,
												"FX Layering",
												FXLayeringWin_X - BtnSizeManual * 3 - 23,
												Inner_Spacing,
												Disable,
												Lbl_Clickable,
												"Bottom",
											...
											if Lyr.Selected[FXGUID_RackMixer] == LyrID then
												...
												for FX_Idx = 1, Sel_Track_FX_Count - 1, 1 do
													if FX[FXGUID[FX_Idx]] then
														if
															FX[FXGUID[FX_Idx]].inWhichLyr == LyrID
															and FX.InLyr[FXGUID[FX_Idx]] == FXGUID_RackMixer
														then
															r.ImGui_DrawList_AddLine(
																WDL,
																R - 2 + Pad,
											...
											if r.ImGui_IsItemClicked(ctx) and Mods == Alt then
												...
												for FX_Idx = 1, Sel_Track_FX_Count - 1, 1 do
													if
														FX[FXGUID[FX_Idx]].inWhichLyr
															== FX[FXGUID_RackMixer].LyrID[LyrID]
														and FX.InLyr[FXGUID[FX_Idx]] == FXGUID_RackMixer
													then
														TheresFXinLyr = true
													end
												...
												function DeleteOneLayer(LyrID, FxGUID, FX_Idx, LT_Track)
													...
													FX[FxGUID].ActiveLyrCount =
														math.max(FX[FxGUID].ActiveLyrCount - 1, 1)
										...
										elseif Lyr.Rename[LyrID .. FxGUID] == true then
											...
											FX[FxGUID].LyrTitle = FX[FxGUID].LyrTitle or {}
											_, FX[FxGUID].LyrTitle[ID] =
												r.ImGui_InputText(ctx, "##" .. LyrID, FX[FxGUID].LyrTitle[ID])
										...
										then
											...
											if
												r.ImGui_Button(ctx, "(n) No (or Esc)")
												or r.ImGui_IsKeyPressed(ctx, 78)
												or r.ImGui_IsKeyPressed(ctx, 27)
											then
												r.ImGui_CloseCurrentPopup(ctx)
											end
											...
											if r.ImGui_Button(ctx, "(y) Yes") or r.ImGui_IsKeyPressed(ctx, 89) then
												...
												for i = 1, HowMany, 1 do
													if
														FX[FXGUID[L]].inWhichLyr
															== FX[FXGUID_RackMixer].LyrID[LyrID]
														and FX.InLyr[FXGUID[L]] == FXGUID_RackMixer
													then
										...
										if
											Lyr.Selected[FXGUID_RackMixer] == LyrID
											and Lyr.Rename[LyrID .. FxGUID] ~= true
										then
											r.ImGui_DrawList_AddRect(
												drawlist,
												ProgBar_Pos_L,
										...
										if Lyr.Solo[LyrID .. FxGUID] == 1 then
											r.ImGui_PopStyleColor(ctx, SoloBtnClrPop)
										end
										...
										if Lyr.Mute[LyrID .. FxGUID] == 0 then
											r.ImGui_PopStyleColor(ctx, LyrMuteClrPop)
										end
								...
								if FX[FxGUID].ActiveLyrCount ~= 8 then
									...
									if AddNewLayer then
										...
										if FX[FxGUID].ActiveLyrCount <= 8 then
											local EmptyChan, chan1, chan2, chan3
											local lastnum = 0
											...
											if not EmptyChan then
												EmptyChan = FX[FxGUID].ActiveLyrCount + 1
											end
							...
							for FX_Idx_InLayer = 0, Sel_Track_FX_Count - 1, 1 do
								...
								for LayerNum, LyrID in pairs(FX[FxGUID].LyrID) do
									...
									if FX.InLyr[FXGUID_To_Check_If_InLayer] == FXGUID[FX_Idx] then --if fx is in rack mixer
										if FindStringInTable(BlackListFXs, FX.Win_Name[FX_Idx_InLayer]) then
										end

										if Lyr.Selected[FXGUID_RackMixer] == nil then
											Lyr.Selected[FXGUID_RackMixer] = 1
										end
										local FXGUID_LayerCheck = r.TrackFX_GetFXGUID(LT_Track, FX_Idx_InLayer)
										if
											FX[FXGUID[FX_Idx_InLayer]].inWhichLyr
												== FX[FXGUID_RackMixer].LyrID[LyrID]
											and LyrID == Lyr.Selected[FXGUID_RackMixer]
											and not FindStringInTable(BlackListFXs, FX.Win_Name[FX_Idx_InLayer])
										then
											r.ImGui_SameLine(ctx, nil, 0)
								...
								if FX.InLyr[FXGUID_To_Check_If_InLayer] == FXGUID[FX_Idx] then
									if Lyr.FrstFXPos[FXGUID_RackMixer] == nil then
										...
										Lyr.FrstFXPos[FXGUID_RackMixer] =
											math.min(Lyr.FrstFXPos[FXGUID_RackMixer], FX_Idx_InLayer)
									end
							...
							if HowManyFXinLyr > 0 and FX[FxGUID].TheresFXinLyr then -- ==  Add and theres fx in selected layer
								...
								for FX_Idx = 1, Sel_Track_FX_Count - 1, 1 do
									if FX[FXGUID[FX_Idx]] then
										if
											FX[FXGUID[FX_Idx]].inWhichLyr
												== FX[FXGUID_RackMixer].LyrID[Lyr.Selected[FXGUID_RackMixer]]
											and FX.InLyr[FXGUID[FX_Idx]] == FXGUID_RackMixer
										then
											TheresFXinLyr = true
										end
						...
						then
							...
							if
								r.ImGui_Button(ctx, "(c) Cancel  (or Esc)")
								or r.ImGui_IsKeyPressed(ctx, 67)
								or r.ImGui_IsKeyPressed(ctx, 27)
							then
								FX[FxGUID].DeleteFXLayer = nil
					...
					elseif FX_Name:find("FXD Saike BandSplitter") then
						...
						for i, v in ipairs(FXGUID) do
							if FX[FxGUID].AttachToJoiner == v then
								JoinerID = i
							end
						end
						...
						if r.ImGui_BeginChild(ctx, "FXD Saike BandSplitter" .. FxGUID, Width, 220) then
							...
							if FX[FxGUID].Collapse then
								...
								for i = 1, Cuts * 4, 1 do ----------[Repeat for Bands]----------
									...
									if --[[Hovering over a band]]
										r.ImGui_IsMouseHoveringRect(ctx, WinL, Cross_Pos - 3, WinR, Cross_Pos + 3)
									then
										...
										elseif IsRBtnClicked then
											...
											for I, v in ipairs(FX[FxGUID].FXsInBS) do
												if FX[v].InWhichBand >= i then
													...
													--sets input channel
													r.TrackFX_SetPinMappings(
														LT_Track,
														Fx,
														0,
														0,
														2 ^ ((FX[v].InWhichBand + 1) * 2 - 2),
														0
													)
													r.TrackFX_SetPinMappings(
														LT_Track,
									...
									--ttp('BD1='..BD1..'\nBD2='..BD2..'\nBD3='..BD3..'\nBD4='..BD4)
									if --[[Mouse is between bands]]
										r.ImGui_IsMouseHoveringRect(ctx, WinL, X.Pos, WinR, NxtCrossPos)
									then
										if Payload_Type == "FX_Drag" then
										end
									end
									...
									if
										r.ImGui_IsMouseHoveringRect(ctx, WinL, WinT, WinR, WinT + H) and IsRBtnClicked
									then
									end
										...
										if IsLBtnHeld then
											...
											if DragDeltaY > 0 or DragDeltaY < 0 then
												...
												for i = 1, 4 - B, 1 do
													if
														PrmV - DragDeltaY / 250
														> r.TrackFX_GetParamNormalized(LT_Track, FX_Idx, B + i)
													then
														r.TrackFX_SetParamNormalized(
															LT_Track,
															FX_Idx,
								...
								function DropFXintoBS(FxID, FxGUID_BS, Band, Pl, DropDest, DontMove) --Pl is payload    --!!!! Correct drop dest!!!!
									...
									if not DontMove then
										...
										if Pl > FX_Idx and not DropDest then
											DropDest = FX_Idx + 1
										end
									...
									r.GetSetMediaTrackInfo_String(
										LT_Track,
										"P_EXT: FX is in which BS" .. FxID,
										FxGUID_BS,
										true
									)
									r.GetSetMediaTrackInfo_String(
										LT_Track,
										"P_EXT: FX is in which Band" .. FxID,
										Band,
										true
									)
								...
								for i = 0, Cuts * 4, 1 do ------- Rpt for Spaces between band splits
									...
									if i == 0 then
										...
										CrossPos = FX[FxGUID].Cross[math.min(i, 4)].Pos
									end
									if i == Cuts * 4 then
										Nxt_CrossPos = WinT
									else
										Nxt_CrossPos = FX[FxGUID].Cross[i + 1].Pos
									end
									local HvrOnBand =
										r.ImGui_IsMouseHoveringRect(ctx, WinL, CrossPos - 3, WinR, CrossPos + 3)
									local HvrOnNxtBand =
										r.ImGui_IsMouseHoveringRect(ctx, WinL, Nxt_CrossPos - 3, WinR, Nxt_CrossPos + 3)

									if --[[Hovering over a band]]
										r.ImGui_IsMouseHoveringRect(ctx, WinL, Nxt_CrossPos, WinR, CrossPos)
										and not (HvrOnBand or HvrOnNxtBand)
									then
										local function Find_InsPos()
											local InsPos
											for I, v in ipairs(FX[FxGUID].FXsInBS) do
												if FX[v].InWhichBand == i then
													InsPos = tablefind(FXGUID, v)
												end
											end
											Pl = Pl or InsPos
											if not InsPos then
										...
										elseif Payload_Type == "AddFX_Sexan" then
											...
											if r.ImGui_IsMouseReleased(ctx, 0) then
												local InsPos = Find_InsPos()
												local rv, type, payload, is_preview, is_delivery =
													r.ImGui_GetDragDropPayload(ctx)
										...
										elseif r.ImGui_IsKeyPressed(ctx, r.ImGui_Key_S()) and Mods == 0 then
											...
											if Mute == 1 then
												local OnOff =
													r.TrackFX_GetParamNormalized(LT_Track, JoinerID, 4 + 5 * i)
												local V
												if OnOff == 1 then
													V = 0
												else
										...
										elseif r.ImGui_IsKeyPressed(ctx, r.ImGui_Key_M()) and Mods == Shift then
											...
											for i = 0, Cuts * 4, 1 do
												...
												if OnOff == 0 then
													table.insert(FX[FxGUID].PreviouslyMutedBand, i)
												end
										...
										elseif r.ImGui_IsKeyPressed(ctx, r.ImGui_Key_S()) and Mods == Shift then
											...
											for i = 0, Cuts * 4, 1 do
												...
												if OnOff == 1 then
													table.insert(FX[FxGUID].PreviouslySolodBand, i)
												end
										...
										elseif IsRBtnClicked and Cuts ~= 1 then
											...
											if Cuts == 0.75 then
												if Norm_V > X[3].Val then
													Seg = 5
												end
											elseif Cuts == 0.5 then
												if Norm_V > X[2].Val then
													Seg = 5
												end
										...
										elseif
											LBtn_MousdDownDuration > 0.06
											and (Mods == 0 or Mods == Apl)
											and not DraggingFXs.SrcBand
											and FX[FxGUID].StartCount
										then
										...
										elseif
											DraggingFXs.SrcBand and DraggingFXs[1] and IsLBtnHeld
											or Payload_Type == "FX_Drag"
										then
											FX[FxGUID].Sel_Band = i
										end
											...
											elseif not IsLBtnHeld and Mods == Apl then
												...
												for I, v in ipairs(DraggingFXs) do
													...
													if not tablefind(FX[FxGUID].FXsInBS, ID) then
														table.insert(FX[FxGUID].FXsInBS, ID)
													end
													FX[ID] = FX[ID] or {}
													FX[ID].InWhichBand = i
													r.GetSetMediaTrackInfo_String(
														LT_Track,
														"P_EXT: FX is in which Band" .. ID,
														i,
														true
													)
													r.GetSetMediaTrackInfo_String(
														LT_Track,
														"P_EXT: FX is in which BS" .. ID,
														FxGUID,
														true
													)
									...
									if JoinerID then
										...
										local Clr = Layer_Mute or CustomColorsDefault.Layer_Mute
										if Pwr == 0 then
											r.ImGui_DrawList_AddRectFilled(WDL, WinL, Nxt_CrossPos, WinR, CrossPos, Clr)
										end
								...
								if
									r.ImGui_BeginPopupModal(
										ctx,
										"Delete Band" .. (FX[FxGUID].PromptDeleteBand or "") .. "? ##" .. FxGUID,
										nil,
										r.ImGui_WindowFlags_NoTitleBar() | r.ImGui_WindowFlags_NoResize()
									)
								then
								...
								for i = 1, Cuts * 4, 1 do
									...
									if FX[FxGUID].Cross.HoveringBand == i or FX[FxGUID].Cross.DraggingBand == i then
										...
										if
											not r.ImGui_IsMouseHoveringRect(
												ctx,
												WinL,
												FX[FxGUID].Cross.HoveringBandPos - 3,
												WinR,
												FX[FxGUID].Cross.HoveringBandPos + 3
											)
											or (FX[FxGUID].Cross.DraggingBand == i and not IsLBtnHeld)
										then
								...
								for i = 0, Cuts * 4, 1 do
									if FXCountForBand[i] or 0 > 0 then
										...
										if
											i == 4
											or (i == 3 and Cuts == 0.75)
											or (i == 2 and Cuts == 0.5)
											or (i == 1 and Cuts == 0.25)
										then
											nxt_X_Pos = WinT
										...
										if This_B_Pos - nxt_X_Pos > 28 and not DraggingFXs[1] then
											r.ImGui_DrawList_AddTextEx(
												...
												14,
												WinL + 10,
												nxt_X_Pos + (This_B_Pos - nxt_X_Pos - 10) / 2,
												0xffffff66,
												FXCountForBand[i] or ""
											)
						...
						if not FX[FxGUID].Collapse then
							...
							for FX_ID = 0, Sel_Track_FX_Count, 1 do
								for i, v in ipairs(FX[FxGUID].FXsInBS) do
									...
									if FXGUID[FX_ID] == v and FX[FxGUID].Sel_Band == FX[v].InWhichBand then
										if FxName:find("FXD ReSpectrum") then
											ofs = ofs + 1
										end
							...
							if LastFX_XPos then
								...
								if AddTopLine then
									r.ImGui_DrawList_AddLine(WDL, WinL, WinT, WinR, WinT, Clr)
								end
								if FX[FxGUID].Sel_Band == 0 then
									r.ImGui_DrawList_AddLine(WDL, WinL, WinT + H, WinR, WinT + H, Clr)
								end
						...
						then
							...
							if r.ImGui_Button(ctx, "(n) No") or r.ImGui_IsKeyPressed(ctx, 78) then
								...
								for i = 0, Sel_Track_FX_Count, 1 do
									if tablefind(FX[FxGUID].FXsInBS, FXGUID[i]) then
										...
										r.GetSetMediaTrackInfo_String(
											LT_Track,
											"P_EXT: FX is in which Band" .. FXGUID[i],
											"",
											true
										)
							...
							SL()
							if
								r.ImGui_Button(ctx, "(esc) Cancel") or r.ImGui_IsKeyPressed(ctx, r.ImGui_Key_Escape())
							then
								FX[FxGUID].DeleteBandSplitter = nil
								r.ImGui_CloseCurrentPopup(ctx)
							end
					...
					if
						Trk[TrkID].PreFX[1]
						and not Trk[TrkID].PreFX_Hide
						and FX_Idx == #Trk[TrkID].PreFX - 1 + offset
					then
				...
				if Sel_Track_FX_Count == 0 then
					AddSpaceBtwnFXs(0, false, true)
				end
							...
							if FxGUID then
								for P, v in ipairs(FX[FxGUID]) do
									...
									if FP.WhichCC then
										for m = 1, 8, 1 do
											if FP.ModAMT[m] then
												r.gmem_write(1000 * m + P, FP.ModAMT[m])
											end
										end
									end
								end
			...
			if (Payload_Type == "FX_Drag" and MouseAtRightEdge) and not Trk[TrkID].PostFX[1] then
				...
				if r.ImGui_BeginDragDropTarget(ctx) then -- if drop to post fx chain
					...
					if --[[Move FX out of layer]]
						Drop and FX.InLyr[FXGUID[DragFX_ID]]
					then
						FX.InLyr[FXGUID[DragFX_ID]] = nil
						r.SetProjExtState(
							0,
			...
			if Trk[TrkID].PostFX[1] then
				...
				if r.ImGui_IsItemClicked(ctx, 1) then
					if Trk[TrkID].PostFX_Hide then
						Trk[TrkID].PostFX_Hide = false
					else
						Trk[TrkID].PostFX_Hide = true
					end
				end
				...
				if not Trk[TrkID].PostFX_Hide then
					if r.ImGui_BeginChild(ctx, "Post FX chain", PostFX_Width - PostFX_Extend_W, 220) then
						...
						for FX_Idx, V in pairs(Trk[TrkID].PostFX) do
							...
							local Spc
							if FX_Idx == 1 and I then
								AddSpaceBtwnFXs(I - 1, "SpcInPost", nil, nil, 1)
							end
							if I then
								createFXWindow(I)
								r.ImGui_SameLine(ctx, nil, 0)
								...
								if
									FX_Idx == #Trk[TrkID].PostFX
									and r.ImGui_IsItemHovered(ctx, r.ImGui_HoveredFlags_RectOnly())
								then
		...
		if LT_Track then
			FXCountEndLoop = r.TrackFX_GetCount(LT_Track)
		end
		r.ImGui_End(ctx)
	end --end for Visible
